...
import { createMusicControllerState, enqueue, next, previous, playTrack, toggleShuffle, cycleRepeat, setOfflineOnly, type MusicControllerState, type Track, createMusicLibraryState, toggleFavorite, recordPlayed, markDownloaded, type MusicLibraryState, createNowPlayingState, openNowPlaying, closeNowPlaying, setNowPlayingExpanded, type NowPlayingState } from "@jhadina/music-core";
...
export default function MusicPage() {
  ...
  const [nowPlaying, setNowPlaying] = useState<NowPlayingState>(() => createNowPlayingState());
  ...
  async function search() {
    ...
    setLoading(true);
    try { const response = await fetch(`/api/music/search?q=${encodeURIComponent(query)}`); const body = await response.json(); const nextResults = body.data?.results ?? []; setResults(nextResults); setLibrary((current) => ({ ...current, tracks: [...new Map([...current.tracks, ...nextResults.map((item: Result) => item.track)].map((track) => [track.id, track])).values()] })); }
    finally { setLoading(false); }
  }

  function play(item: Result | Track) {
    const track = "track" in item ? item.track : item;
    const result = "track" in item ? item : results.find((candidate) => candidate.track.id === track.id);
    if (result?.sourceUri) setSources((current) => ({ ...current, [track.id]: result.sourceUri! }));
    setLibrary((current) => recordPlayed(current, track.id));
    setMusic((current) => playTrack(current, track));
    setNowPlaying((current) => openNowPlaying(current, track));
  }

  function addToQueue(item: Result | Track) { const track = "track" in item ? item.track : item; const result = "track" in item ? item : results.find((candidate) => candidate.track.id === track.id); if (result?.sourceUri) setSources((current) => ({ ...current, [track.id]: result.sourceUri! })); setMusic((current) => enqueue(current, [track])); }
  ...
  return <main className="min-h-screen bg-[#07080b] text-white">
    ...
    {nowPlaying.isOpen && nowPlaying.track && <NowPlayingPanel track={nowPlaying.track} lyrics={nowPlaying.lyrics} playing={music.playing} positionMs={music.positionMs} favorite={favoriteIds.has(nowPlaying.track.id)} downloaded={downloadedIds.has(nowPlaying.track.id)} onTogglePlay={() => setMusic((current) => ({ ...current, playing: !current.playing }))} onSeek={(positionMs) => setMusic((current) => ({ ...current, positionMs }))} onFavorite={() => favorite(nowPlaying.track!.id)} onDownload={() => download(nowPlaying.track!.id)} onClose={() => setNowPlaying((current) => closeNowPlaying(current))} />}
    <div className="mx-auto max-w-7xl px-5 pb-36 pt-7 md:px-10 md:pt-10">
      ...
      {results.length > 0 && <section className="mt-10"><div className="mb-4 flex items-center justify-between"><p className="text-[11px] uppercase tracking-[.3em] text-white/30">Search Results</p><button onClick={() => setMusic((current) => setOfflineOnly(current, !current.offlineOnly))} className={`rounded-full border px-3 py-1.5 text-xs ${music.offlineOnly ? "border-white/30 bg-white/10" : "border-white/10 text-white/40"}`}>{music.offlineOnly ? "Offline only" : "All sources"}</button></div><TrackList tracks={results.map((item) => item.track)} favoriteIds={favoriteIds} downloadedIds={downloadedIds} onPlay={play} onQueue={addToQueue} onFavorite={favorite} onDownload={download} /></section>}
      {tab !== "home" && <section className="mt-8"><div className="mb-5"><p className="text-[11px] uppercase tracking-[.3em] text-white/30">{tab}</p><h2 className="mt-2 text-3xl font-semibold">{tab === "favorites" ? "Your Favorites" : tab === "recent" ? "Recently Played" : tab === "downloads" ? "Available Offline" : tab === "playlists" ? "Your Playlists" : "Your Library"}</h2></div>{tab === "playlists" ? <div className="rounded-3xl border border-white/10 bg-white/[.035] p-8 text-white/40">Playlists are ready for provider sync in the next library slice.</div> : <TrackList tracks={visibleTracks} favoriteIds={favoriteIds} downloadedIds={downloadedIds} onPlay={play} onQueue={addToQueue} onFavorite={favorite} onDownload={download} />}</section>}
    ...
    <div className="fixed inset-x-0 bottom-0 z-30 border-t border-white/10 bg-[#090a0e]/92 px-4 py-3 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center gap-3"><button onClick={() => currentTrack && setNowPlaying((current) => openNowPlaying(current, currentTrack))} className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-white/10">♪</button><button onClick={() => currentTrack && setNowPlaying((current) => openNowPlaying(current, currentTrack))} className="min-w-0 flex-1 text-left"><p className="truncate text-sm font-medium">{currentTrack?.title ?? "Nothing playing"}</p><p className="truncate text-xs text-white/35">{subtitle}</p></button><button onClick={() => setMusic((current) => toggleShuffle(current))} disabled={!currentTrack} className={`hidden rounded-xl px-2 text-xs sm:block ${music.shuffle ? "text-white" : "text-white/30"}`}>↝</button><button onClick={() => setMusic((current) => previous(current))} disabled={!currentTrack} className="text-white/60 disabled:opacity-20">◀◀</button><button onClick={() => setMusic((current) => ({ ...current, playing: !current.playing }))} disabled={!currentTrack || !sourceUri} className="grid h-11 w-11 place-items-center rounded-full bg-white text-black disabled:opacity-30">{music.playing ? "Ⅱ" : "▶"}</button><button onClick={() => setMusic((current) => next(current))} disabled={!currentTrack} className="text-white/60 disabled:opacity-20">▶▶</button><button onClick={() => setMusic((current) => cycleRepeat(current))} disabled={!currentTrack} className="hidden rounded-xl px-2 text-xs text-white/40 sm:block">{music.repeat === "one" ? "↻1" : music.repeat === "all" ? "↻" : "↻·"}</button><button onClick={() => setQueueOpen(!queueOpen)} className="rounded-xl border border-white/10 px-3 py-2 text-xs text-white/60">Queue {music.queue.length}</button></div></div>
  </main>;
}
