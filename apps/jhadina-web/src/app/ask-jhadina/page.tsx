 ...
 async function ask(commandOverride?:string, conversationSignals?:JhadinaConversationSignals){
  ...
  try{
   ...
   if(referenceFile){
    ...
    const response=await fetch("/api/jhadina/command",{method:"POST",headers:{"content-type":"application/json","x-jhadina-user-id":userId},body:JSON.stringify({activeTask:command,surface,route,artifacts,conversationSignals,activeProject:params.get("project")??undefined,clientRequestId:crypto.randomUUID()})})
