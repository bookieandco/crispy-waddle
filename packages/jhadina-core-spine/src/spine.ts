...
import type { KnowledgePort, KnowledgeRecord } from './knowledge.js';
...
export interface ContextPort { build(input: { experience: Experience; memories: MemoryProposal[]; patterns: PatternObservation[]; personality: PersonalityState; knowledge: readonly KnowledgeRecord[] }): Promise<ContextPacket>; }
...
export interface SpinePorts { memory: MemoryPort; pattern: PatternPort; personality: PersonalityPort; context: ContextPort; decision: DecisionPort; policy: PolicyPort; action: ActionPort; audit: AuditPort; evolution: EvolutionPort; knowledge: KnowledgePort; }
export interface SpineRunResult { memories: MemoryProposal[]; patterns: PatternObservation[]; personality: PersonalityState; knowledge: readonly KnowledgeRecord[]; context: ContextPacket; decision: DecisionProposal; policy: PolicyDecision; action?: ActionRequest; result?: ActionResult; }

/** Jhadina's control-plane orchestration boundary. Providers implement ports; the spine owns ordering and policy-before-action. */
export class JhadinaSpine {
  ...
  async run(experience: Experience): Promise<SpineRunResult> {
    ...
    const knowledge = await this.ports.knowledge.retrieve({ text: experience.content, limit: 20 });
    const context = await this.ports.context.build({ experience, memories: combinedMemories, patterns, personality, knowledge });
    const decision = await this.ports.decision.decide(context);
    const policy = await this.ports.policy.evaluate(decision);
    await this.ports.audit.record({ type: policy.allowed ? 'DECISION_AUTHORIZED' : 'POLICY_DENIED', actor: 'jhadina', subjectId: decision.id, payload: { proposalId: decision.id, allowed: policy.allowed, reason: policy.reason } });
    if (!policy.allowed) return { memories, patterns, personality, knowledge, context, decision, policy };
    const action = await this.ports.action.prepare(decision, policy);
    if (!action) return { memories, patterns, personality, knowledge, context, decision, policy };
    ...
    return { memories, patterns, personality, knowledge, context, decision, policy, action, result };
  }
