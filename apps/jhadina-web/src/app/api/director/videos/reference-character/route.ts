...
export async function POST(request: Request) {
  ...
  const { data: castRow, error: castError } = await privileged
  ...
  if (castError) return NextResponse.json({ ok: false, error: castError.message }, { status: 500 });
  if (!castRow) return NextResponse.json({ ok: false, error: 'DIRECTOR_CAST_NOT_FOUND' }, { status: 404 });

  const cast = castRow.cast_record as CharacterCastRecord;
  const castErrors = validateCharacterCastRecord(cast);
  if (castErrors.length) {
    return NextResponse.json({ ok: false, error: `DIRECTOR_CAST_INVALID:${castErrors.join(';')}` }, { status: 409 });
  }
