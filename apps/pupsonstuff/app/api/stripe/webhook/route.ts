...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!items || items.length !== rawItems.length) {
      return NextResponse.json({ success: false, error: "Stripe line-item catalog metadata failed validation." }, { status: 422 });
    }
    ...
    return NextResponse.json({ success: false, error: "Order persistence failed; Stripe should retry this event." }, { status: 500 });
  }
}
