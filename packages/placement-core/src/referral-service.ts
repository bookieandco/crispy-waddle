...
export class ReferralConsentError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "ReferralConsentError";
  }
}
...
export function projectReferral(
  ...
  if (consent.id !== referral.consentId) {
    throw new ReferralConsentError("Referral is not linked to the supplied consent record");
  }

  if (consent.workerId !== referral.workerId || consent.agencyId !== referral.agencyId || consent.jobId !== referral.jobId) {
    throw new ReferralConsentError("Consent subject does not match referral");
  }

  if (consent.status !== "GRANTED") {
    throw new ReferralConsentError("Worker consent has not been granted");
  }

  const scopes = new Set(consent.scopes);
  const approvedProfile: ReferralProjection["approvedProfile"] = {};
