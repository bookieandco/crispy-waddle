...
function verifyIdentity(identityVerifier: JhadinaIdentityVerifier, claimedUserId?: string): Promise<ActionRequestIdentity> {
...
export async function proposeCommerceAction(
  ...
  if (decision === "deny") {
    await deps.ledger.append({
      id: `${provisionalId}:denied`, actionId: provisionalId, userId: identity.userId,
      type: COMMERCE_PAYMENT_CHARGE_CAPABILITY, status: "denied", timestamp: now(),
    })
...
export async function executeCommerceProposal(
  ...
  if (!valid) {
    await deps.ledger.append({
      id: `${proposal.id}:execution-denied`, actionId: proposal.id, userId: identity.userId,
      type: proposal.capability, status: "denied", timestamp: now(),
  ...
  const intent = await governedProvider.createPaymentIntent({
    paymentId: proposal.id, orderId: proposal.id,
