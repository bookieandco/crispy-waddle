...
export async function persistSharkLaunch(client: SupabaseClient, collection: SolanaLaunchCollection) {
  ...
  const { data: canonicalLaunchId, error } = await client.rpc('jhadina_shark_persist_launch_bundle', {
    p_launch: launchRow,
    p_edges: edges,
  })
  if (error) throw new Error(`SHARK atomic launch persistence failed: ${error.message}`)

  return { launchId: String(canonicalLaunchId ?? launch.launchId), persistedEdges: edges.length }
}
