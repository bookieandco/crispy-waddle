...
export class RemoteQualityMonitor {
 ...
 async sample(nowMs=Date.now()):Promise<RemoteQualityMonitorSnapshot>{const sample=await this.probe();const decision=evaluateRemoteQuality(sample,this.policy);const degradedScore=this.options.degradedScore??70;const state:RemoteQualityState=!decision.allowed?'blocked':decision.score<degradedScore?'degraded':'healthy';const snapshot:RemoteQualityMonitorSnapshot={sample,decision,state,atMs:nowMs};this.latest=snapshot;this.onChange?.(snapshot);return snapshot;}
