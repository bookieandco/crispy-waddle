...
export function buildCapitalDecisionCenter(
  ...
  if (liquidity.action === 'withdraw' && liquidity.amount > 0) {
    alerts.push({
      id: 'liquidity-protection', severity: 'critical',
      title: 'Protect operating liquidity',
      message: `Cash is below the configured safety target by approximately ${liquidity.amount}.`,
      recommendedAction: `Review a withdrawal of up to ${liquidity.amount} from liquid investments.`,
      requiresHumanApproval: true,
    });
  } else if (liquidity.action === 'deposit' && liquidity.amount > 0) {
    alerts.push({
      id: 'excess-cash', severity: 'medium',
      title: 'Excess liquidity detected',
      message: `Approximately ${liquidity.amount} may be deployable after configured reserves.`,
      recommendedAction: `Review depositing/deploying up to ${liquidity.amount}.`,
      requiresHumanApproval: true,
    });
  }
  ...
  opportunities.slice(0, 5).forEach((opportunity) => {
    ...
    if (opportunity.action === 'consider' && allocation && allocation.recommendedAmount > 0) {
      alerts.push({
        id: `opportunity-${opportunity.id}`,
        severity: opportunity.confidence >= 0.8 ? 'high' : 'medium',
        title: `${opportunity.domain}: ${opportunity.instrument}`,
        message: `Positive risk-adjusted opportunity with confidence ${(opportunity.confidence * 100).toFixed(0)}%.`,
