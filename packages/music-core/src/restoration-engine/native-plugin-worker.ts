...
export interface NativePluginWorker {
  handle(request: NativePluginIpcRequest): Promise<NativePluginIpcResponse>;
}
...
export function createNativePluginWorker(binding: NativePluginIpcBinding, backend: NativePluginBackend): NativePluginWorker {
  ...
  const response = (request: NativePluginIpcRequest, extra: Partial<NativePluginIpcResponse> = {}): NativePluginIpcResponse => ({
    protocol: NATIVE_PLUGIN_IPC_PROTOCOL,
    version: NATIVE_PLUGIN_IPC_VERSION,
  ...
  const error = (request: NativePluginIpcRequest, e: unknown): NativePluginIpcResponse => ({
    ...
    type: "error",
    state,
    error: { code: "NATIVE_WORKER_FAILURE", message: e instanceof Error ? e.message : "Native plugin worker request failed." },
  });

  return {
    async handle(request) {
      validateNativePluginIpcRequest(request);
      if (request.binding.jobId !== binding.jobId || request.binding.executionId !== binding.executionId || request.binding.sourceHash !== binding.sourceHash || request.binding.plugin.binaryHash !== binding.plugin.binaryHash || request.binding.automationPlanHash !== binding.automationPlanHash) {
        throw new Error("Native plugin worker binding mismatch.");
      }
      assertWorkerCannotAuthorizeOrPromote(request.type);
      try {
        switch (request.type) {
          case "discover":
            discovered = await backend.discover(binding.inputPath, binding);
            state = "DISCOVERED";
            return response(request, { metadata: { plugin: discovered, hostVersion: "jhadina-native-host", runtimeVersion: "1.0.0", format: binding.plugin.format, state } });
          case "load":
            if (!discovered) throw new Error("Plugin must be discovered before load.");
            await backend.load(request.pluginPath, binding);
            state = "LOADED";
            return response(request);
          case "configure":
            await backend.configure(request.audio);
            state = "CONFIGURED";
            return response(request);
          case "set_automation":
            await backend.setAutomation(request);
            state = "AUTOMATION_BOUND";
            return response(request);
          case "process_block":
            await backend.processBlock(request.sampleOffset, request.numSamples);
            state = "PROCESSING";
            return response(request);
          case "flush":
            await backend.flush();
          ...
          case "collect_metadata":
            state = "COMPLETED";
            return response(request, { metadata: await backend.collectMetadata() });
          case "shutdown":
