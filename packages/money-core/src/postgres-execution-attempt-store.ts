...
import type { ExecutionAttempt, ExecutionAttemptOutcome, ExecutionAttemptStore, ExecutionAttemptState, ExecutionActionSnapshot } from './execution-attempt.js';

type AttemptRow = {
  attempt_id: string; request_id: string; permit_id: string; action_fingerprint: string;
  action_snapshot: ExecutionActionSnapshot; provider: string; operation: string;
  idempotency_key: string; state: ExecutionAttemptState; provider_reference: string | null;
  error_code: string | null; error_message: string | null; started_at: string | Date;
  completed_at: string | Date | null; recovery_required: boolean;
};
...
function mapRow(row: AttemptRow): ExecutionAttempt {
  return {
    attemptId: row.attempt_id, requestId: row.request_id, permitId: row.permit_id,
    actionFingerprint: row.action_fingerprint, actionSnapshot: Object.freeze({ ...row.action_snapshot }),
    provider: row.provider, operation: row.operation, idempotencyKey: row.idempotency_key,
    state: row.state, providerReference: row.provider_reference ?? undefined,
    errorCode: row.error_code ?? undefined, errorMessage: row.error_message ?? undefined,
    startedAt: iso(row.started_at)!, completedAt: iso(row.completed_at), recoveryRequired: row.recovery_required,
  };
}
...
export class PostgresExecutionAttemptStore implements ExecutionAttemptStore {
  ...
  async start(attempt: ExecutionAttempt): Promise<void> {
    await this.client.query(`INSERT INTO ${this.table} (attempt_id, request_id, permit_id, action_fingerprint, action_snapshot, provider, operation, idempotency_key, state, started_at, recovery_required) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'STARTED',$9,FALSE)`, [attempt.attemptId, attempt.requestId, attempt.permitId, attempt.actionFingerprint, JSON.stringify(attempt.actionSnapshot), attempt.provider, attempt.operation, attempt.idempotencyKey, attempt.startedAt]);
  }
  ...
  async get(attemptId: string): Promise<ExecutionAttempt | undefined> {
    const result = await this.client.query<AttemptRow>(`SELECT attempt_id, request_id, permit_id, action_fingerprint, action_snapshot, provider, operation, idempotency_key, state, provider_reference, error_code, error_message, started_at, completed_at, recovery_required FROM ${this.table} WHERE attempt_id=$1 LIMIT 1`, [attemptId]);
