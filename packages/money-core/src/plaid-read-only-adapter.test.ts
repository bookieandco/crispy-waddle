...
const credentials = JSON.stringify({ clientId: 'client-test', secret: 'secret-test', accessToken: 'access-sandbox-test' });
...
try {
  ...
  globalThis.fetch = async (input, init) => {
    ...
    return new Response(JSON.stringify({
      accounts: [{ account_id: 'acct-123', name: 'Checking', mask: '4242', type: 'depository', subtype: 'checking', balances: { iso_currency_code: 'USD' } }],
    }), { status: 200, headers: { 'content-type': 'application/json' } });
  };

  const adapter = new PlaidReadOnlyAdapter({ baseUrl: 'https://sandbox.plaid.com', credentialBundle: credentials });
  const accounts = await adapter.listAccounts({ userId: 'user-test', capability: 'money.account.read', requestId: 'req-test' });
  ...
  try {
    await adapter.listTransactions({ userId: 'user-test', capability: 'money.transaction.read', requestId: 'req-test' }, 'plaid:acct-123');
  } catch (error) {
  ...
  try { new PlaidReadOnlyAdapter({ baseUrl: 'https://sandbox.plaid.com', credentialBundle: '{}' }); } catch { invalidRejected = true; }
