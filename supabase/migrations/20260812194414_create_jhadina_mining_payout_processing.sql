...
begin
  ...
  set last_scanned_height = p_block_height,
      last_scanned_hash = p_block_hash,
      last_successful_scan_at = p_scanned_at,
      updated_at = now()
  ...
  if not found then
    insert into public.jhadina_mining_scan_checkpoints (
      network, receiving_address, last_scanned_height, last_scanned_hash,
      last_successful_scan_at, scanner_version, reorg_lookback
    ) values (
