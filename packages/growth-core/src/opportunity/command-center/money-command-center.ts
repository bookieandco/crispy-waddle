...
export function projectMoneyCommandCenter(entries: readonly QueueEntry[], generatedAt = new Date().toISOString()): MoneyCommandCenterSnapshot {
  const cards = entries.map(({ opportunity, rank }) => {
    ...
    const evidenceConfidence = opportunity.score?.evidenceConfidence ?? 0
    const bucket: MoneyCommandBucket = score >= 75 && evidenceConfidence >= 70 ? 'pursue' : score >= 50 && evidenceConfidence >= 45 ? 'test' : 'watch'
