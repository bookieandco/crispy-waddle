...
export function planResearchCase(input: {
  ...
  const base = createMoneyResearchCase({ opportunityId: input.action.opportunityId, title: input.title, now: input.now })
  ...
  return {
    ...
    focusBranches,
    branches: base.branches.map((item) => ({ ...item, status: focus.has(item.kind) ? "READY" : "PENDING" })),
  }
}

export function planResearchQueue(actions: MoneyActionItem[], titles: Record<string, string> = {}): PlannedResearchCase[] {
  return actions.flatMap((action) => {
    const planned = planResearchCase({ action, title: titles[action.opportunityId] || `SAM opportunity ${action.opportunityId}` })
