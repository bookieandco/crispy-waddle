...
import { OpportunityDiscoveryService } from './opportunity-discovery.js'
...
describe('OpportunityDiscoveryService', () => {
  it('normalizes valid provider output into the repository', async () => {
    ...
    const service = new OpportunityDiscoveryService(repository, [{ source, async discover() { return [{ opportunity: opportunity(), source }] } }])
