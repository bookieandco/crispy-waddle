...
export default function GrowthCommandCenter() {
  ...
  async function load() {
    ...
    try {
      const res = await fetch("/api/growth/drafts", { cache: "no-store" })
  ...
  async function action(path: string, draftId: string, extra: Record<string, string> = {}) {
    ...
    try {
      const res = await fetch(path, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ draftId, ...extra }) })
