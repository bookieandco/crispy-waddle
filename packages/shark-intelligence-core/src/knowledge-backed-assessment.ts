...
export type SharkKnowledgeBackedAssessment = {
  ...
  strategyId: string
  knowledgeIds: string[]
  status: 'KNOWLEDGE_BACKED' | 'ASSESSMENT_WITH_UNCERTAINTY' | 'INSUFFICIENT_KNOWLEDGE'
...
export function assessWithSharkKnowledge(input: {
  ...
  if (relevant.length === 0) {
    return { assessmentId: input.assessmentId, opportunityId: input.opportunityId, strategyId: input.strategyId, knowledgeIds: [], status: 'INSUFFICIENT_KNOWLEDGE', relevanceScore: 0, confidence: 0, uncertainty, provenanceComplete: false, simulated: true, paperOnly: true }
  }
  ...
  return { assessmentId: input.assessmentId, opportunityId: input.opportunityId, strategyId: input.strategyId, knowledgeIds: relevant.map(item => item.knowledgeId), status, relevanceScore, confidence, uncertainty, provenanceComplete: true, simulated: true, paperOnly: true }
}
