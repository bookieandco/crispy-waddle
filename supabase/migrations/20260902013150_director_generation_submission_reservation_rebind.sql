...
begin
  ...
  if not found then raise exception 'generation task not found: %', p_task_id; end if;
  select * into v_execution from public.director_generation_executions where id = p_execution_id for update;
  if not found or v_execution.task_id <> p_task_id then raise exception 'generation execution does not belong to task: %', p_execution_id; end if;
  if v_execution.lease_owner is distinct from p_lease_owner or v_execution.lease_token is distinct from p_lease_token or v_execution.lease_expires_at is null or v_execution.lease_expires_at <= clock_timestamp() then
    raise exception 'stale or expired generation execution lease: %', p_execution_id;
  end if;
  select * into v_existing from public.director_generation_submission_outbox where provider_id = p_provider_id and idempotency_key = p_idempotency_key for update;
  if found then
    if v_existing.task_id <> p_task_id then raise exception 'submission idempotency key belongs to another generation task'; end if;
    if v_existing.status = 'submitted' then return v_existing; end if;
    update public.director_generation_submission_outbox
      set execution_id = p_execution_id, request_payload = p_request_payload,
          lease_owner = p_lease_owner, lease_token = p_lease_token,
          lease_expires_at = v_execution.lease_expires_at, updated_at = clock_timestamp()
  ...
  insert into public.director_generation_submission_outbox (
    ...
    status, attempt, lease_owner, lease_token, lease_expires_at, created_at, updated_at
  ) values (
    p_task_id, p_execution_id, p_provider_id, p_idempotency_key, p_request_payload,
    'pending', 0, p_lease_owner, p_lease_token, v_execution.lease_expires_at, clock_timestamp(), clock_timestamp()
  ) returning * into v_row;
  return v_row;
exception
  when unique_violation then
    select * into v_existing from public.director_generation_submission_outbox where provider_id = p_provider_id and idempotency_key = p_idempotency_key;
    if found and v_existing.task_id = p_task_id then return v_existing; end if;
    raise;
end;
$$;
