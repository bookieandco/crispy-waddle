...
type ClipPresentation = { name?: string; kind?: 'video' | 'audio'; fade?: { fadeInSeconds: number; fadeOutSeconds: number; curve: FadeCurve } };
type Clip = TimelineClip & ClipPresentation;
...
function clipPresentation(clip: TimelineClip): ClipPresentation {
  return clip as TimelineClip & ClipPresentation;
}

function clipDisplayName(clip: TimelineClip): string {
  const name = clipPresentation(clip).name?.trim();
  return name || clip.assetId;
}

function clipFade(clip: TimelineClip): ClipPresentation['fade'] {
  return clipPresentation(clip).fade;
}
