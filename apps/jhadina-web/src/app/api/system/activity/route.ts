...
export async function GET(request:Request){
  ...
  try{
    ...
    return NextResponse.json({ok:true,data:{events,verifiedUserId}})
  }catch(error){
    return NextResponse.json({error:error instanceof Error?error.message:"Unable to read activity"},{status:403})
  }
}
