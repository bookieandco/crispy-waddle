...
function lockedTraitsFor(ctx: PromptContext): string[] {
  ...
  return (ctx.entities ?? []).filter((entity) => handles.has(entity.id) || handles.has(entity.name)).flatMap((entity) => entity.lockedTraits);
}
...
function materialReferencesFor(ctx: PromptContext): string[] {
  const handles = new Set(ctx.shot.entityHandles);
  return (ctx.refs ?? []).filter((ref) => handles.has(ref.entityId)).map((ref) => `@material[${ref.entityId}]: ${describeReference(ref)} — ${ref.uri}`);
}
...
export function emitPrompts(ctx: PromptContext, targets: PromptTarget[] = promptTargets): Record<string, string> {
  const out: Record<string, string> = {};
  for (const target of targets) out[target.name] = applyLookPreset(target.render(ctx), ctx.shot.director?.lookPreset);
  return out;
}
