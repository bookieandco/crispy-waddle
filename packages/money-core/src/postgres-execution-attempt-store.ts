...
import type { ExecutionAttempt, ExecutionAttemptStore, ExecutionAttemptState } from './execution-attempt.js';
...
export class PostgresExecutionAttemptStore implements ExecutionAttemptStore {
  ...
  constructor(options: PostgresExecutionAttemptStoreOptions) {
    this.client = options.client;
    this.table = tableIdentifier(options.tableName ?? 'money_execution_attempts');
  }

  async start(attempt: ExecutionAttempt): Promise<void> {
    await this.client.query(
      `INSERT INTO ${this.table} (
        attempt_id, request_id, permit_id, action_fingerprint, provider,
        operation, idempotency_key, state, started_at, recovery_required
      ) VALUES ($1,$2,$3,$4,$5,$6,$7,'STARTED',$8,FALSE)`,
      [attempt.attemptId, attempt.requestId, attempt.permitId, attempt.actionFingerprint, attempt.provider, attempt.operation, attempt.idempotencyKey, attempt.startedAt],
    );
  }
  ...
  async complete(
    ...
    await this.client.query(
      `UPDATE ${this.table}
       SET state=$2, provider_reference=$3, error_code=$4, error_message=$5,
           recovery_required=$6, completed_at=$7, updated_at=CURRENT_TIMESTAMP
       WHERE attempt_id=$1 AND state='STARTED'`,
      [attemptId, outcome.state, outcome.providerReference ?? null, outcome.errorCode ?? null, outcome.errorMessage ?? null, outcome.recoveryRequired, completedAt],
    );
  }

  async get(attemptId: string): Promise<ExecutionAttempt | undefined> {
    const result = await this.client.query<AttemptRow>(
      `SELECT attempt_id, request_id, permit_id, action_fingerprint, provider,
              operation, idempotency_key, state, provider_reference,
              error_code, error_message, started_at, completed_at, recovery_required
       FROM ${this.table} WHERE attempt_id=$1 LIMIT 1`,
      [attemptId],
    );
