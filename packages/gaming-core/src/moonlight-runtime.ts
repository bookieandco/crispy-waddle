...
export class MoonlightRemoteRuntime implements GameRuntimeAdapter {
  ...
  async launch(game:Game,input:LaunchContext):Promise<GameSession>{if(!(await this.canLaunch(game)))throw new Error(`Moonlight cannot launch ${game.platform}:${game.id}`);const [,hostId,appId]=game.contentUri.split('://')[1]?.split('/')??[];if(!hostId||!appId)throw new Error('Moonlight URI must be moonlight://<host>/<app>');const session=await this.client.launch(hostId,appId,input);return{id:session.id,gameId:game.id,runtimeId:this.runtime.id,startedAt:new Date().toISOString()};}
