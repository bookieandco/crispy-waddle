...
export function rankSharkContextualRecall(input: {
  ...
  const scored = input.candidates.map(candidate => {
    ...
    const score = candidate.relevance * 0.35 + candidate.confidence * 0.25 + candidate.recency * 0.15 + candidate.relationshipStrength * 0.25 - contradiction * 0.2
    return { experienceId: candidate.experienceId, score: Math.max(0, Math.min(1, score)) }
  })
  return scored
