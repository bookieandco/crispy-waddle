...
export function evaluateDirectorGenerationGate(input: DirectorGenerationGateInput): DirectorGenerationGateDecision {
  ...
  try {
    authoritative = deriveDirectorCreativeProvenance({ sequence, binding, storyboardStage: input.storyboardStage, generationStage: input.generationStage, storyboardBoardId: board.id });
  } catch (error) {
    return { allowed: false, reason: error instanceof Error ? error.message : 'Unable to establish authoritative storyboard lineage.' };
  }

  if (!sameCreativeProvenance({ ...input.creativeProvenance, generationJobId: 'pending-generation-job' }, { ...authoritative, generationJobId: 'pending-generation-job' })) return { allowed: false, reason: 'Creative provenance does not match authoritative storyboard lineage.' };
