...
export class PlacementFinancialService {
  constructor(private readonly repository:PlacementFinancialRepository, private readonly invoiceLedger:InvoiceLedgerService, private readonly resolution:AgreementResolutionService, private readonly idempotency?:FinancialIdempotencyGuard) {}
  ...
  async finalize(input:PlacementFinancialInput):Promise<PlacementFinancialResult>{
    ...
    return {invoice,agreementId:resolved.agreement.id,contractId:resolved.contract.id,basisAmount:resolved.split.basisAmount,platformFee:resolved.split.platformFee,agencyRevenue:resolved.split.agencyRevenue,platformRevenue:resolved.split.platformRevenue};
  }
}
