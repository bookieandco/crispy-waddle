...
export async function recordDirectorTakeExperience(
  ...
  const event: ExperienceEvent = {
    ...
    metadata: { projectId: projectId ?? null, shotId: feedback.shotId, takeId: feedback.takeId, reaction: feedback.reaction, notes: feedback.notes ?? null, controls: controls ?? null },
  };
  return experiences.append(event);
}

export async function recordGeneratedDirectorTakeExperience(
  take: { takeId: string; shotId: string; clipUri: string; provider: string; occurredAt: string; projectId?: string; controls?: DirectorControls },
  experiences: ExperiencePort,
  ownerId: string,
): Promise<Awaited<ReturnType<ExperiencePort['append']>>> {
  const event: ExperienceEvent = {
    ...
    metadata: { projectId: take.projectId ?? null, shotId: take.shotId, takeId: take.takeId, clipUri: take.clipUri, provider: take.provider, controls: take.controls ?? null },
  };
  return experiences.append(event);
}
