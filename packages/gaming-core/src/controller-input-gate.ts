...
export interface ControllerInputGateResult {allowed:boolean;reason:ControllerInputGateReason;deviceId:string;sessionId:string;health?:ControllerHealthResult;healthObservedAtMs?:number;}
...
export class ControllerInputGate {
  ...
  constructor(private readonly bindings:ControllerSessionBindingManager,policy:ControllerInputGatePolicy=DEFAULT_CONTROLLER_HEALTH_GATE_POLICY){if(!Number.isFinite(policy.maxHealthAgeMs)||policy.maxHealthAgeMs<0)throw new Error('maxHealthAgeMs must be finite and non-negative');this.policy={...policy};}
  updateHealth(result:ControllerHealthResult,observedAtMs=Date.now()):void{if(!Number.isFinite(observedAtMs))throw new Error('observedAtMs must be finite');const binding=this.bindings.get();if(!binding||binding.state!=='bound'||binding.deviceId!==result.deviceId)throw new Error('Controller health requires an active session binding');this.health.set(result.deviceId,{result,observedAtMs,sessionId:binding.sessionId});}
  ...
  authorize(event:InputIntegrityEvent,nowMs=Date.now()):ControllerInputGateResult{
    ...
    const deviceId=event.deviceId;const sessionId=event.sessionId;
    ...
    let binding;try{binding=this.bindings.assertBound(sessionId,deviceId);}catch{return{allowed:false,reason:'controller-unbound',deviceId,sessionId};}
    if(binding.sessionId!==sessionId||binding.deviceId!==deviceId)return{allowed:false,reason:'session-mismatch',deviceId,sessionId};
    const envelope=this.health.get(deviceId);if(!envelope)return{allowed:false,reason:'controller-unhealthy',deviceId,sessionId};
    ...
    try{assertControllerCapabilities({deviceId,capabilities:binding.capabilities},{required:requiredCapabilities});}catch{return{allowed:false,reason:'controller-capability-mismatch',deviceId,sessionId,health,healthObservedAtMs:observedAtMs};}
