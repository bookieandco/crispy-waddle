...
create table if not exists public.jhadina_evolution_run_ledger (
  ...
  constraint jhadina_evolution_run_ledger_type_check check (
    type in ('RUN_STARTED','RUN_DISPATCHED','RUN_VERIFIED','RUN_FAILED','RUN_BLOCKED','DRAFT_PR_CREATED')
  ),
...
create policy "authenticated users can read evolution run events"
  ...
  for select
  to authenticated
  using ((payload ->> 'actor_id')::uuid = (select auth.uid()));

create or replace function public.reject_jhadina_evolution_run_ledger_mutation()
returns trigger
language plpgsql
security definer
...
as $$
begin
  raise exception 'jhadina evolution run ledger is append-only';
end;
$$;
...
create or replace function public.verify_jhadina_evolution_run_ledger(p_run_id bigint)
returns boolean
language plpgsql
security definer
set search_path = public, extensions, pg_temp
...
begin
  for r in
    select run_id, sequence, event_id, task_id, type, occurred_at, payload, previous_hash, hash
      from public.jhadina_evolution_run_ledger
     where run_id = p_run_id
     order by sequence
  loop
