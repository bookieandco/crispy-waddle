...
export type JhadinaCommand = {
  id: string;
  text: string;
  source: 'button' | 'voice' | 'keyboard' | 'system';
  surface: JhadinaSurface;
  context?: Record<string, unknown>;
  createdAt: string;
};

export type JhadinaIntent = {
  name: string;
  confidence: number;
...
export type JhadinaPlan = {
  commandId: string;
  summary: string;
  steps: JhadinaPlanStep[];
};

export type JhadinaResult = {
...
export function registerJhadinaCapability(name: string, handler: JhadinaHandler) {
...
export async function dispatchJhadinaCommand(command: JhadinaCommand): Promise<JhadinaResult> {
  ...
  if (!handler) {
    return { status: 'needs_approval', message: `I understand the request, but the ${capability} capability is not connected yet.` };
  }
