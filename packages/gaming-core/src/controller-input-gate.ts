...
export class ControllerInputGate {
  ...
  authorize(event:InputIntegrityEvent,nowMs=Date.now()):ControllerInputGateResult{
    ...
    const required=[...(this.policy.requiredCapabilities??[]),CONTROLLER_INPUT_CAPABILITY[event.inputKind]];
    try{assertControllerCapabilities({deviceId,capabilities:binding.capabilities},{required:[...new Set(required)]});}
