...
export interface SourceAssetResolver { resolve(source: MusicSource, track: Track): Promise<MediaAsset | null>; }

/** Resolves playback only after canonical user ownership and source authorization checks. */
export class UserScopedPlaybackResolver implements AuthorizedPlaybackResolver {
  ...
  async resolve(request: UserPlaybackRequest): Promise<UserPlaybackResult | null> {
    ...
    const assets = await this.repository.listAssets(request.userId, track.id);
    const local = assets.find((asset) => asset.kind === "file" && asset.provenance?.offline === true);
    ...
    for (const source of sources) {
      ...
      if (asset && asset.sourceId === source.id && asset.trackId === track.id) return { track, asset, mode: "network" };
    }
    return null;
  }
}
