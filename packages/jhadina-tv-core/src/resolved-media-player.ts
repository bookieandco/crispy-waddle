...
function assertResolvedPlayback(playback: ResolvedPlaybackSource): void {
...
export function createResolvedMediaPlayer(
  ...
  assertResolvedPlayback(playback);
  runtime.setSource(playback.source.url);
  return { ...runtime, playback };
}

export function assertNativePlaybackSource(playback: ResolvedPlaybackSource): void {
  assertResolvedPlayback(playback);
}
