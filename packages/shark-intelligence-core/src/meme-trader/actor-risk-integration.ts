...
export type ActorRiskIntelligence = { launchBehavior?: LaunchBehaviorSignal[]; lpControlRisk?: LPControlRisk; rugProtection?: RugProtectionResult; top10HolderPct?: number; bundledSupplyRisk?: number; profitableTrackedWallets?: number; accumulatingWallets?: number; distributingWallets?: number; evidenceIds?: string[] }
...
export function applyActorRiskIntelligence(assessment: MemeTradeAssessment, actor: ActorRiskIntelligence): MemeTradeAssessment {
  ...
  const weightedBadRate = totalConfidence ? signals.reduce((sum, s) => sum + (s.rugRate ?? 0) * s.confidence, 0) / totalConfidence : 0
  const deployerRisk = clamp(weightedBadRate)
  ...
  risk.overallRisk = clamp(.15 * risk.marketIntegrity + .12 * risk.liquidityRisk + .14 * risk.supplyControlRisk + .10 * risk.holderConcentrationRisk + .08 * risk.walletCohortRisk + .10 * risk.socialManipulationRisk + .07 * risk.narrativeFragilityRisk + .06 * risk.developerRisk + .05 * risk.contractRisk + .05 * risk.networkRisk + .04 * (1 - risk.attentionQuality) + .04 * risk.exitLiquidityRisk)
  risk.band = risk.overallRisk >= .8 ? 'blocked' : risk.overallRisk >= .6 ? 'high-risk' : risk.overallRisk >= .4 ? 'watch' : 'candidate'

  const evidenceIds = [...new Set([...assessment.evidenceIds, ...(actor.evidenceIds ?? []), ...signals.flatMap(s => s.evidenceIds), ...(actor.lpControlRisk?.evidenceIds ?? []), ...(actor.rugProtection?.evidenceIds ?? [])])]
  return { ...assessment, supplyControl, holderCohort, riskAssessment: risk, evidenceIds, assessmentVersion: 'meme-trader-assessment-v2-actor-risk' }
}
