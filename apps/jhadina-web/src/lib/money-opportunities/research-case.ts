export type ResearchBranchKind =
  | "AGENCY" | "REQUIREMENTS" | "INCUMBENT" | "COMPETITORS" | "PARTNERS" | "ECONOMICS" | "NEXT_ACTION"
export type ResearchBranchStatus = "PENDING" | "READY" | "COMPLETE" | "BLOCKED"
export interface ResearchBranch { id: string; kind: ResearchBranchKind; question: string; status: ResearchBranchStatus; findings: string[] }
export interface MoneyResearchCase { id: string; opportunityId: string; source: "sam.gov"; title: string; createdAt: string; branches: ResearchBranch[] }
function branch(id: string, kind: ResearchBranchKind, question: string): ResearchBranch { return { id, kind, question, status: "PENDING", findings: [] } }
export function createMoneyResearchCase(input: { opportunityId: string; title: string; now?: Date }): MoneyResearchCase {
  const prefix = input.opportunityId || "opportunity"
  return { id: `research-${prefix}`, opportunityId: input.opportunityId, source: "sam.gov", title: input.title, createdAt: (input.now || new Date()).toISOString(), branches: [
