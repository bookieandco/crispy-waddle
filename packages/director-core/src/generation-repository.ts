...
export class InMemoryGenerationRepository implements GenerationRepository {
  ...
  async claimExecution(taskId: string, providerId: string, workerId: string, leaseMs: number): Promise<GenerationExecution | undefined> {
    const id = `${taskId}:attempt:1`; const now = Date.now(); const existing = this.executions.get(id);
    ...
    const updated: GenerationExecution = existing ? { ...existing, providerId, leaseOwner: workerId, leaseToken: newLeaseToken(), leaseExpiresAt: new Date(now + leaseMs).toISOString(), updatedAt: new Date(now).toISOString() } : { id, taskId, providerId, attempt: 1, status: 'queued', leaseOwner: workerId, leaseToken: newLeaseToken(), leaseExpiresAt: new Date(now + leaseMs).toISOString(), createdAt: new Date(now).toISOString(), updatedAt: new Date(now).toISOString() };
    this.executions.set(id, clone(updated)); return clone(updated);
  }
  ...
  async reserveSubmission(task: GenerationTask, execution: GenerationExecution, providerId: string, idempotencyKey: string): Promise<GenerationSubmissionOutbox | undefined> {
    const durableTask = this.tasks.get(task.id); const durableExecution = this.executions.get(execution.id);
    ...
    const key = `${providerId}:${idempotencyKey}`; const existingId = this.submissionsByKey.get(key);
    if (existingId) {
      const existing = this.submissions.get(existingId); if (!existing) return undefined;
      if (existing.taskId !== task.id) throw new Error('Submission idempotency key belongs to another generation task');
      if (existing.status === 'submitted') return clone(existing);
      const rebound = { ...existing, executionId: execution.id, requestPayload: clone(task.request), leaseOwner: execution.leaseOwner, leaseToken: execution.leaseToken, leaseExpiresAt: execution.leaseExpiresAt, updatedAt: new Date().toISOString() };
      this.submissions.set(existing.id, clone(rebound)); return clone(rebound);
    }
    const now = new Date().toISOString(); const submission: GenerationSubmissionOutbox = { id: newSubmissionId(), taskId: task.id, executionId: execution.id, providerId, idempotencyKey, requestPayload: clone(task.request), status: 'pending', attempt: 0, leaseOwner: execution.leaseOwner, leaseToken: execution.leaseToken, leaseExpiresAt: execution.leaseExpiresAt, createdAt: now, updatedAt: now };
    this.submissions.set(submission.id, clone(submission)); this.submissionsByKey.set(key, submission.id); return clone(submission);
  }
