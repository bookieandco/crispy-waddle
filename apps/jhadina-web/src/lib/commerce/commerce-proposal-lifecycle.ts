...
export interface CommerceProposalExecutionDeps extends CommerceProposalLifecycleDeps { paymentProvider: PaymentProvider }
...
function verifyIdentity(identityVerifier: JhadinaIdentityVerifier, claimedUserId: string | undefined): Promise<ActionRequestIdentity> {
  return identityVerifier.verify(claimedUserId === undefined ? undefined : { userId: claimedUserId })
}
...
export async function proposeCommerceAction(
  ...
  const decision = await policy.evaluate({ id: provisionalId, userId: identity.userId, type: COMMERCE_PAYMENT_CHARGE_CAPABILITY, action: { capability: COMMERCE_PAYMENT_CHARGE_CAPABILITY }, requestedAt: now() })
  if (decision === "deny") {
  ...
  await deps.ledger.append({ id: `${created.id}:proposed`, actionId: created.id, userId: identity.userId, type: COMMERCE_PAYMENT_CHARGE_CAPABILITY, status: "started", timestamp: now(), metadata: { decision, fingerprint, amountMinor: payload.amountMinor, currency: payload.currency } })
...
export async function approveCommerceProposal(
  ...
  const request = { id: proposal.id, userId: identity.userId, type: proposal.capability, action: { capability: proposal.capability, fingerprint }, requestedAt: now() }
  ...
  await deps.ledger.append({ id: `${proposal.id}:approved`, actionId: proposal.id, userId: identity.userId, type: proposal.capability, status: "completed", timestamp: now(), metadata: { stage: "human_approval", approvalReceiptId: approved.id, expiresAt: approved.expiresAt } })
...
export async function executeCommerceProposal(
  ...
  const valid = await verifier.verifyAndConsume(proposal.receiptId, { id: proposal.id, userId: identity.userId, type: proposal.capability, action: { capability: proposal.capability, fingerprint }, requestedAt: now() })
  if (!valid) {
    await deps.ledger.append({ id: `${proposal.id}:execution-denied`, actionId: proposal.id, userId: identity.userId, type: proposal.capability, status: "denied", timestamp: now(), metadata: { reason: "invalid_expired_or_replayed_approval_receipt" } })
  ...
  const intent = await governedProvider.createPaymentIntent({
    paymentId: proposal.id, orderId: proposal.id, customer: { id: identity.userId, type: "customer" }, seller: { id: "jhadina-commerce", type: "platform" },
    amount: { amountMinor: proposal.payload.amountMinor, currency: proposal.payload.currency }, lines: [], taxes: [], platformFees: [],
    metadata: { stripeTestPaymentMethod: proposal.payload.testPaymentMethod, description: proposal.payload.description },
  })
  const updated = await deps.proposalStore.markExecuted(proposal.id, identity.userId, { paymentId: intent.paymentId, provider: intent.provider, providerReference: intent.providerReference ?? null, status: intent.status })
  return { proposal: updated, verifiedUserId: identity.userId, paymentId: intent.paymentId, providerReference: intent.providerReference, status: intent.status }
}
