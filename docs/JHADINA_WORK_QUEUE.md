...
**Priority:** P2
**Status:** REVIEW
**Branch:** `fix/money-final-deploy-gates`
...
**Security boundary:** PR #472's encrypted-token column was further hardened by the MONEY-FINAL audit: encrypted Plaid access tokens are now split into `jhadina_money_bank_credentials`, a service-role-only RLS table. Owner-readable connection/account metadata contains no provider secret. Bank Link writes/disconnects and per-Item adapter construction are server-only and explicitly bound to the verified user.
**Verification (2026-09-20):** Money R13B Certification run #218 SUCCESS; Spatial Conformance run #745 SUCCESS; Jhadina Evolution Core CI #1257 SUCCESS; Staffing Postgres Integration #2001 SUCCESS; Director Targeted Tests #163 SUCCESS; SHARK Intelligence Core CI #111 SUCCESS. Jhadina Web Deploy Conformance #95 fails on unrelated missing `@/lib/director-review-transition-repository`; Launch Gate fails on unrelated `@jhadina/tv-core` unused `assertTVEpisode`. Live Jhadina Supabase schema was exercised with RLS enabled and security advisors returned no Money-specific findings after removing SECURITY DEFINER bypasses.\n**Post-merge audit (2026-09-20):** PR #472 is merged as `0a8fdfc4`. Full repository/handoff audit is recorded in `docs/architecture/MONEY-FINAL-audit-and-acceptance.md`. It also repaired the undeclared Action Core workspace dependency and hardened Plaid credential persistence.\n**Post-audit closure (2026-09-20):** PR #478 merged as `785f84f`; Money R13B #231 and Spatial #764 are green on its audited head. Live Jhadina Supabase re-verification confirms RLS on connection/account/credential tables, authenticated SELECT-only access to owner-readable metadata, no authenticated credential-table grant, and no Money-specific Security Advisor finding. PR #485 repairs the unrelated Director/TV/workstation build defects exposed while restoring repository-wide deploy gates and records the broader handoff gaps accurately.\n**Human gate:** Merge PR #485 after its Web Deploy/Launch Gate receipts are green, deploy the resulting main with server-only `SUPABASE_SERVICE_ROLE_KEY`, `JHADINA_MONEY_CREDENTIAL_KEY` (32-byte base64), and `JHADINA_SECRET_PLAID_DEFAULT`, then run one authenticated Plaid sandbox Link -> owned account read -> transaction read -> durable audit -> disconnect -> post-disconnect denial drill. Until those deployment/commissioning receipts exist, MONEY-FINAL remains REVIEW and must not be represented as production-accepted.
