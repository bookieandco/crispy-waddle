...
| **Result / Approval Receipt** | `ApprovalReceipt` / `InMemoryApprovalReceiptStore` | ✅ Full create/approve/consume semantics with expiry and binding | ⚠️ In-memory singleton in production | ⚠️ Non-durable — lost on restart | N/A | N/A | ✅ Actor + fingerprint + expiry binding | ✅ Consume audited | ✅ Adversarial tests added (this PR) | ⚠️ | Durable atomic implementation required for production; tracked Issue #193 | Durable approval receipt store (Issue #193) |
...
- `packages/jhadina-action-core/src/approval-receipt-adversarial.test.ts` (new) — 9 adversarial tests
