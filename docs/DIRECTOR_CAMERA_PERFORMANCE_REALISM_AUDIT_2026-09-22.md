...
- `StoryboardBoard` and storyboard→take handoff carry camera, performance, realism, and animation-principles plans in the contract layer. The repo now includes a migration adding `camera_plan`, `performance_plan`, `realism_plan`, and `animation_plan` JSONB columns to canonical boards and immutable board-version history, and the Supabase read adapter hydrates them. Remote migration deployment remains a runtime-certification step.
- `buildStoryboardShotPlan` validates and carries the latest structured plans.
- `TakeRequest` carries camera, performance, realism, and animation-principles plans.
- `compileTakePrompt` translates structured direction into provider-facing sections, including an explicit `[ANIMATION PRINCIPLES]` block when present.
...
1. Apply and verify the committed storyboard direction-plan migration in the target Supabase environment, including `animation_plan`; repository-level schema/read-path support is implemented, remote deployment is not yet certified.
