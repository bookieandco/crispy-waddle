...
export class SupabaseDirectorCharacterReferenceAssetResolver implements DirectorCharacterReferenceAssetResolver {
  constructor(
  ...
  async resolve(assetId: string, projectId: string): Promise<{ uri: string; sha256?: string; mimeType?: string }> {
    const { data, error } = await this.client
      ...
      .select('id,project_id,bucket_id,object_path,sha256,mime_type,admission_status,scan_status')
      .eq('id', assetId)
