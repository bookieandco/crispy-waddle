...
test('rejects generation overflow',async()=>{const root=base('root');const p1=base('p1',{recoveryOfExecutionId:'root'});const p2=base('p2',{recoveryOfExecutionId:'p1'});await rejects(p2,base('c',{recoveryOfExecutionId:'p2'}),'GENERATION_LIMIT',[p1,root])})
