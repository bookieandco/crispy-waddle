...
import type { WebScoutPipeline } from './web-scout-pipeline.js';
...
/** One governed operation from web discovery through evidence and cultural promotion. */
export class ResearchInvestigation {
  ...
  async investigate(query: string, limit = 5, now = new Date().toISOString()): Promise<InvestigationResult> {
    ...
    for (const signal of discovered.signals) {
      ...
      const evidence = await this.evidence.record(checked);
      ...
      if (evidence.duplicate) duplicates += 1;
      results.push({ signal: checked, verification: checked.verification, evidence: evidence.evidence, promoted: Boolean(promotion), duplicate: evidence.duplicate });
    }
    return { query, discovered: discovered.discovered, crawled: discovered.crawled, rejected: discovered.rejected, verified, corroborated, promoted, duplicates, signals: results };
  }
}
