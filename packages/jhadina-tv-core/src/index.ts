...
export function recommendTitles(catalog: MediaTitle[], request: RecommendationRequest): RecommendationResult[] {
  ...
  return catalog.map((title) => {
    ...
    if (queryMatches.length) { score += Math.min(45, queryMatches.length * 15); reasons.push(`Matches your search for ${queryMatches.join(', ')}`); }
    const genreMatches = title.genres.filter((genre) => wantedGenres.has(normalize(genre)));
    if (genreMatches.length) { score += genreMatches.length * 15; reasons.push(`Matches ${genreMatches.join(' and ')} preferences`); }
    if (request.maxRuntimeMinutes && title.runtimeMinutes) {
      if (title.runtimeMinutes <= request.maxRuntimeMinutes) { score += 10; reasons.push(`Fits your ${request.maxRuntimeMinutes}-minute limit`); } else score -= 20;
    }
    const related = signals.find((signal) => signal.titleId === title.id);
    if (related?.liked) { score += 20; reasons.push('Builds on something you liked'); }
    if (title.availability !== 'external-link') { score += 5; reasons.push('Available through a configured JhadinaTV source'); }
    return { title, score, reasons };
  }).filter((result) => result.score > 0).sort((a, b) => b.score - a.score);
}

export const JHADINA_TV_ROUTES = {
  home: '/jhadinatv', movies: '/jhadinatv/movies', tv: '/jhadinatv/tv', search: '/jhadinatv/search',
