import type { Opportunity, OpportunityClaim, OpportunityEvidence } from '@jhadina/opportunity-core'
...
export function assessOpportunity({ opportunity, kind = 'other', streetSignals = [], modelVersion }: SharkOpportunityAssessmentInput): SharkOpportunityDecision {
  ...
  for (const claim of opportunity.claims) { const risk = riskFromClaim(claim); if (risk) risks.add(risk) }
...
export function validateSharkDecision(decision: SharkOpportunityDecision): string[] {
  ...
  for (const [name, value] of [['confidence', decision.confidence], ['sourceQuality', decision.sourceQuality], ['novelty', decision.novelty], ['repeatability', decision.repeatability]] as const) if (!Number.isFinite(value) || value < 0 || value > 1) errors.push(`${name} must be between 0 and 1`)
...
export type { OpportunityEvidenceAdapter } from './evidence-adapter.js'
