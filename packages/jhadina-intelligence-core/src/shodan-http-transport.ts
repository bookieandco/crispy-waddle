...
export type ShodanHttpClient = (url: string, init?: { headers?: Record<string, string> }) => Promise<{
  ok: boolean
  status: number
...
export class ShodanHttpTransport implements ShodanTransport {
  ...
  async read(capability: ShodanReadCapability, subject: string): Promise<unknown> {
    ...
    const response = await this.http(endpoint(capability, subject, key), { headers: { Accept: 'application/json' } })
