...
export async function POST(request: Request) {
  ...
  if (action === "upsert") {
    ...
    if (
      candidate.userId !== userId || candidate.providerId !== providerId || candidate.itemId !== itemId ||
      !isFiniteNonNegativeNumber(candidate.positionMs) ||
      (durationMs !== undefined && !isFiniteNonNegativeNumber(durationMs)) ||
      (typeof durationMs === "number" && candidate.positionMs > durationMs) ||
      typeof candidate.completed !== "boolean" || !isIsoTimestamp(candidate.updatedAt) ||
  ...
  try {
    ...
    const saved = await repository.upsert({
      userId: identity.userId, providerId, itemId,
    ...
    if (message === "Authenticated user missing" || message === "Authenticated session missing") return jsonError("Unauthenticated", 401)
