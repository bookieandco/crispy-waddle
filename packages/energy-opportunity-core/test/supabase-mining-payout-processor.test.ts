...
test('Supabase adapter calls the transactional payout RPC', async () => {
  ...
  const result = await processSupabaseMiningPayout({
    client: { rpc: async (name, args) => { call = { name, args }; return { data: 'processed', error: null }; } },
  ...
  assert.deepEqual(call?.args, {
    ...
    p_output_index: 1, p_amount_sats: 12345, p_block_height: 900, p_block_hash: 'abc',
...
test('duplicate result is preserved', async () => {
  const result = await processSupabaseMiningPayout({
    client: { rpc: async () => ({ data: 'duplicate', error: null }) },
