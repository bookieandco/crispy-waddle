...
describe('cross-chain wallet-cluster swap normalization',()=>{
 it('canonicalizes EVM addresses while preserving raw amounts and evidence time',()=>{
  ...
  expect(r.amountInRaw).toBe('1234567');expect(r.amountInDecimal).toBe('1.234567');expect(r.amountOutDecimal).toBe('2.5')
 ...
 it('does not lowercase Solana identities',()=>{
  ...
  expect(r.walletId).toBe('solana:11111111111111111111111111111111');expect(r.amountInDecimal).toBe('1')
 })
 it('preserves very large amounts exactly instead of rounding them through JavaScript numbers',()=>{
  const r=normalizeCrossChainSwapObservation({evidenceId:'e3',chain:'BASE',transactionId:'t',walletAddress:'0xAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA',tokenIn:'0xBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB',tokenOut:'0xCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC',amountInRaw:'900719925474099300000000',amountOutRaw:'1',tokenInDecimals:6,tokenOutDecimals:0,observedAt:'2026-09-21T20:00:00Z',availableAt:'2026-09-21T20:00:00Z',source:'x'})
  expect(r.amountInDecimal).toBe('900719925474099300')
  expect(r.amountInRaw).toBe('900719925474099300000000')
 })
