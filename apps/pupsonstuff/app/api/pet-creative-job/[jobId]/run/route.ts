...
export async function POST(_request: NextRequest, context: RouteContext) {
  ...
  const { data: job, error: jobError } = await supabase
    ...
    .eq("id", jobId).eq("owner_id", user.id).single();

  if (jobError || !job) return errorResponse("Creative job not found.", 404);
  if (job.status === "completed") {
    const { data: existingOutputs } = await supabase
      .from("pupson_creative_outputs")
      .select("id, asset_id, metadata")
      .eq("creative_job_id", job.id)
      .eq("owner_id", user.id)
      .order("output_index", { ascending: true });
    const outputs = await Promise.all((existingOutputs ?? []).map(async (output) => {
      const { data: asset } = await supabase.from("pupson_media_assets").select("storage_path").eq("id", output.asset_id).eq("owner_id", user.id).single();
      const { data: signed } = asset ? await supabase.storage.from(BUCKET).createSignedUrl(asset.storage_path, 3600) : { data: null };
      return { id: output.id, assetId: output.asset_id, uri: asset?.storage_path ?? "", signedUrl: signed?.signedUrl };
    }));
  ...
  const { data: claimed, error: claimError } = await supabase
    ...
    .eq("id", job.id).eq("owner_id", user.id).eq("status", "queued")
  ...
  try {
    ...
    if (identityAssetsError || !identityAssets?.length) throw new Error("Pet Identity has no source photos.");

    const primaryAssetId = identityAssets[0].asset_id;
    const { data: asset, error: assetError } = await supabase
      .from("pupson_media_assets")
      .select("id, storage_path, mime_type, metadata")
      .eq("id", primaryAssetId).eq("owner_id", user.id).single();
    if (assetError || !asset) throw new Error("Primary pet asset could not be loaded.");

    const { data: sourceFile, error: downloadError } = await supabase.storage.from(BUCKET).download(asset.storage_path);
    if (downloadError || !sourceFile) throw new Error(`Pet source download failed: ${downloadError?.message ?? "unknown error"}`);
    ...
    const backgroundInstruction = backgroundMode === "keep"
    ...
    const outputs: Array<{ id: string; assetId: string; uri: string; signedUrl?: string }> = [];
    for (let index = 0; index < result.imagesBase64.length; index += 1) {
      ...
      const { error: assetInsertError } = await supabase.from("pupson_media_assets").insert({
        id: assetId, owner_id: user.id, kind: "generated", storage_path: path, mime_type: "image/png",
        width: metadata.width ?? null, height: metadata.height ?? null,
        metadata: { source: "creative-job-runner", creativeJobId: job.id, outputIndex: index, petIdentityId: job.pet_identity_id },
      });
      if (assetInsertError) throw new Error(`Generated asset record failed: ${assetInsertError.message}`);
      const { error: outputInsertError } = await supabase.from("pupson_creative_outputs").insert({
        id: outputId, creative_job_id: job.id, owner_id: user.id, asset_id: assetId,
        output_index: index, status: "available", metadata: { mimeType: "image/png", width: metadata.width, height: metadata.height },
      });
      if (outputInsertError) throw new Error(`Creative output record failed: ${outputInsertError.message}`);
      const { data: signed } = await supabase.storage.from(BUCKET).createSignedUrl(path, 3600);
      outputs.push({ id: outputId, assetId, uri: path, signedUrl: signed?.signedUrl });
    }
    ...
    const { data: completedJob, error: completeError } = await supabase
      ...
      .select("id, status, output_count, completed_at").single();
    ...
    await supabase.from("pupson_creative_jobs").update({
      status: "failed",
      error_message: error instanceof Error ? error.message : "Creative job failed.",
      updated_at: new Date().toISOString(),
    }).eq("id", job.id).eq("owner_id", user.id);
    return errorResponse(error instanceof Error ? error.message : "Creative job failed.", 502);
  }
}
