...
export function runRealCoreSelfTest(): void {
  ...
  const passive = core.observe({ id: 'x2', occurredAt: '2026-08-30T00:00:00Z', source: 'system', content: 'heartbeat received', significance: 'low' });
  if (passive.shouldSpeak) throw new Error('low-significance observation should not force speech');
  if (passive.stance !== 'observe') throw new Error('passive observation should remain observe');
}
