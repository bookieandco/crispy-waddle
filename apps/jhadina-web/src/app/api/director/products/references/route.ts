...
export async function POST(request: Request) {
  ...
  const requestedView = textField(form, 'viewHint') || 'hero';
  const viewHint = VIEWS.has(requestedView) ? requestedView : 'hero';
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : 'DIRECTOR_PRODUCT_REFERENCE_IMAGE_INVALID' },
      { status: 400 },
    );
  }
  ...
  if (existing) return NextResponse.json({ ok: true, deduplicated: true, asset: existing }, { status: 200 });

  const assetId = `product-ref:${crypto.randomUUID()}`;
  ...
  if (uploadError) {
    return NextResponse.json({ ok: false, error: `DIRECTOR_PRODUCT_REFERENCE_UPLOAD_FAILED:${uploadError.message}` }, { status: 502 });
  }
  ...
  if (error) {
    ...
    return NextResponse.json({ ok: false, error: `DIRECTOR_PRODUCT_REFERENCE_RECORD_FAILED:${error.message}` }, { status: 409 });
  }

  return NextResponse.json({ ok: true, deduplicated: false, asset: data, next: 'scan-and-admit' }, { status: 201 });
}
