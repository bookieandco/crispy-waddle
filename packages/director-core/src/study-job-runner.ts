...
export async function runStudyJob(id: string, store: StudyJobStore, effects: StudyJobEffects): Promise<StudyJob | undefined> {
  ...
  try {
    for await (const observation of effects.observe(job)) {
      ...
      job = {
        ...job,
        lastTimeSeconds: Math.max(job.lastTimeSeconds, observation.time.endSeconds),
        observationsSeen: job.observationsSeen + 1,
      };
      if (effects.note) {
        await effects.note(observation);
        job = { ...job, notesCreated: job.notesCreated + 1 };
      }
      if (job.shareWithJhadina && effects.learn) {
        await effects.learn(observation, job);
        job = { ...job, learningCandidatesCreated: job.learningCandidatesCreated + 1 };
      }
      await store.save(job);
    }
