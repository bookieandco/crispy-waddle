...
export interface AlertDeliveryRepository {
  ...
  claimDue(now: string, workerId: string, limit: number): Promise<AlertDeliveryRecord[]>
}
...
export class InMemoryWatchlistRepository implements WatchlistRepository {
  ...
  async get(id: string) { return clone(this.entries.get(id)) }
  async listByUser(userId: string) { return [...this.entries.values()].filter((entry) => entry.userId === userId).map(clone) }
  async save(entry: WatchlistEntry) { this.entries.set(entry.id, clone(entry)); return clone(entry) }
...
export class InMemoryAlertEventRepository implements AlertEventRepository {
  ...
  async get(id: string) { return clone(this.events.get(id)) }
  async listByWatchlistEntry(watchlistEntryId: string) { return [...this.events.values()].filter((event) => event.watchlistEntryId === watchlistEntryId).map(clone) }
  async saveIfAbsent(event: AlertEvent) {
    ...
    this.events.set(event.id, clone(event)); this.fingerprints.set(key, event.id)
...
export class InMemoryAlertDeliveryRepository implements AlertDeliveryRepository {
  ...
  async get(id: string) { return clone(this.records.get(id)) }
  async getByIdempotencyKey(idempotencyKey: string) { const id = this.idempotency.get(idempotencyKey); return id ? clone(this.records.get(id)) : undefined }
  async saveIfAbsent(record: AlertDeliveryRecord) {
    ...
    this.records.set(record.id, clone(record)); this.idempotency.set(record.idempotencyKey, record.id)
  ...
  async update(record: AlertDeliveryRecord) {
    ...
    this.records.set(record.id, clone(record)); this.claims.delete(record.id); return clone(record)
  }
  async claimDue(now: string, workerId: string, limit: number) {
    ...
    for (const record of this.records.values()) {
      ...
      if (record.nextAttemptAt && new Date(record.nextAttemptAt).getTime() > nowMs) continue
      if (this.claims.has(record.id)) continue
      this.claims.set(record.id, workerId)
...
export class InMemoryFeedbackRepository implements FeedbackRepository {
  ...
  async get(id: string) { return clone(this.events.get(id)) }
  async listBySubject(subjectId: string) { return [...this.events.values()].filter((event) => event.opportunityId === subjectId || event.principalId === subjectId).map(clone) }
  async append(event: FeedbackEvent) { if (this.events.has(event.id)) throw new Error(`Duplicate feedback event: ${event.id}`); this.events.set(event.id, clone(event)); return clone(event) }
}
...
export class InMemoryVersionedAssessmentRepository implements VersionedAssessmentRepository {
  ...
  async get(id: string) { return clone(this.assessments.get(id)) }
  async listBySubject(subjectId: string, assessmentType?: string) { return [...this.assessments.values()].filter((assessment) => assessment.subjectId === subjectId && (!assessmentType || assessment.assessmentType === assessmentType)).sort((a, b) => a.assessedAt.localeCompare(b.assessedAt)).map(clone) }
  async append(assessment: VersionedAssessment) { if (this.assessments.has(assessment.id)) throw new Error(`Duplicate assessment: ${assessment.id}`); if (assessment.supersedesId && !this.assessments.has(assessment.supersedesId)) throw new Error(`Assessment supersedes unknown version: ${assessment.supersedesId}`); this.assessments.set(assessment.id, clone(assessment)); return clone(assessment) }
}
