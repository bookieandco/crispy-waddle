...
export interface CredentialLeaseRpcClient {
...
export class RpcCredentialLeaseStore implements CredentialLeaseStore {
  ...
  async create(grant: CredentialGrant): Promise<void> {
    const { error } = await this.client.rpc('create_jhadina_credential_lease', {
      p_lease_id: grant.leaseId,
      p_actor_id: grant.actorId,
      p_worker_id: grant.workerId ?? null,
      p_capability: grant.capability,
      p_provider: grant.provider,
      p_credential_ref: grant.credentialRef,
      p_purpose: grant.purpose,
      p_resource_id: grant.resourceId ?? null,
      p_expires_at: new Date(grant.expiresAt).toISOString(),
      p_max_uses: grant.maxUses,
    });
  ...
  async consume(leaseId: string, request: CredentialLeaseUseRequest, now: number): Promise<boolean> {
    const { data, error } = await this.client.rpc<boolean>('consume_jhadina_credential_lease', {
      ...
      p_worker_id: request.workerId ?? null,
      p_capability: request.capability,
      p_provider: request.provider,
      p_credential_ref: request.credentialRef,
