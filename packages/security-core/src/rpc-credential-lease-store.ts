...
export class RpcCredentialLeaseStore implements CredentialLeaseStore {
  ...
  async create(grant: CredentialGrant): Promise<void> {
    const { error } = await this.client.rpc('create_jhadina_credential_lease', {
      p_lease_id: grant.leaseId, p_request_id: grant.requestId, p_actor_id: grant.actorId, p_worker_id: grant.workerId ?? null, p_capability: grant.capability,
      ...
      p_policy_decision_id: grant.policyDecisionId, p_policy_version: grant.policyVersion,
  ...
  async consume(leaseId: string, request: CredentialLeaseUseRequest, now: number, policyDecisionId: string, policyVersion: string): Promise<boolean> {
    const { data, error } = await this.client.rpc<boolean>('consume_jhadina_credential_lease', {
      p_lease_id: leaseId, p_request_id: request.requestId, p_actor_id: request.actorId, p_worker_id: request.workerId ?? null, p_capability: request.capability,
      ...
      p_now: new Date(now).toISOString(), p_policy_decision_id: policyDecisionId, p_policy_version: policyVersion,
      p_egress_destination: request.egressDestination ?? null,
