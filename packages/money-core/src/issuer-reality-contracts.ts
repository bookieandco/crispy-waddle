...
export type Filing = Readonly<{ filingId: string; issuerId: string; accessionNumber: string; formType: string; filingDate: string; acceptedAt: string; periodEnd?: string; amendmentOf?: string; status: FilingStatus; source: string; sourceHash: string; documentRefs: readonly string[]; xbrlRefs: readonly string[]; evidenceRefs: readonly EvidenceRef[]; provenanceHash: string }>
...
export type FactRevision = Readonly<{ revisionId: string; factId: string; supersedesFactId?: string; revisionType: 'AMENDMENT' | 'RESTATEMENT' | 'CORRECTION' | 'WITHDRAWAL'; revisedAt: string; reason?: string; resolvedByFactId?: string; evidenceRefs: readonly EvidenceRef[]; provenanceHash: string }>
...
export function buildFundamentalState(issuerId: string, facts: readonly FinancialFact[], revisions: readonly FactRevision[], informationCutoff: string, derivedAt: string, methodologyVersion: string, inputSnapshotHash: string, evidenceRefs: readonly EvidenceRef[], provenanceHash: string): FundamentalState {
  ...
  const selected = selectActiveFactsAtCutoff(facts, revisions, informationCutoff); const cutoff = Date.parse(informationCutoff); const relevantRevisions = revisions.filter((revision) => Date.parse(revision.revisedAt) <= cutoff)
  const status: FundamentalStateStatus = selected.length === 0 ? 'INSUFFICIENT_HISTORY' : selected.some((fact) => fact.status === 'CONFLICTING') ? 'DATA_CONFLICTING' : relevantRevisions.some((revision) => (revision.revisionType === 'RESTATEMENT' || revision.revisionType === 'CORRECTION' || revision.revisionType === 'AMENDMENT') && !revision.resolvedByFactId) ? 'RESTATEMENT_PENDING' : 'DATA_COMPLETE'
