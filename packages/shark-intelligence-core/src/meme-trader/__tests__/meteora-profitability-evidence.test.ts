...
describe('Meteora DLMM profitability evidence',()=>{
 ...
 it('admits benchmarked LP-vs-HODL divergence only with closed, complete, valuation-proven evidence',()=>{
  const r=reconcileMeteoraDlmmCashFlowProfitabilityFromState({
   position:'position-1',currency:'USDC',informationCutoff:'2026-09-01T01:00:00Z',state:state(),
   flows:[valuedFlow('deposit','DEPOSIT',100000n),valuedFlow('withdraw','WITHDRAWAL',108000n),valuedFlow('fee','FEE',4000n)],
   benchmark:{benchmarkId:'bench-1',position:'position-1',currency:'USDC',hodlValueMinor:115000n,lpTerminalValueMinor:112000n,methodology:'MATCHED_HODL_COUNTERFACTUAL',observedAt:'2026-09-01T00:00:04Z',availableAt:'2026-09-01T00:00:05Z',evidenceIds:['benchmark:e1']},
  })
 ...
 it('keeps future or unverified benchmarks out of PIT profitability truth',()=>{
  const future=reconcileMeteoraDlmmCashFlowProfitabilityFromState({
   ...
   benchmark:{benchmarkId:'bench-future',position:'position-1',currency:'USDC',hodlValueMinor:115000n,lpTerminalValueMinor:112000n,methodology:'MATCHED_HODL_COUNTERFACTUAL',observedAt:'2026-09-02T00:00:00Z',availableAt:'2026-09-02T00:00:01Z',evidenceIds:['benchmark:future']},
  })
  ...
  const unvalued=reconcileMeteoraDlmmCashFlowProfitability({
   position:'position-1',currency:'USDC',informationCutoff:'2026-09-01T01:00:00Z',positionClosed:true,transactionHistoryComplete:true,
   flows:[nativeFlow('deposit','DEPOSIT',100000n),nativeFlow('withdraw','WITHDRAWAL',110000n)],
   benchmark:{benchmarkId:'bench-2',position:'position-1',currency:'USDC',hodlValueMinor:115000n,lpTerminalValueMinor:112000n,methodology:'MATCHED_HODL_COUNTERFACTUAL',observedAt:'2026-09-01T00:00:04Z',availableAt:'2026-09-01T00:00:05Z',evidenceIds:['benchmark:e2']},
  })
  expect(unvalued.impermanentLossMinor).toBeNull()
  expect(unvalued.impermanentLossStatus).toBe('BENCHMARK_REQUIRED')
 })

 it('does not benchmark direct calls without durable state lineage',()=>{
  const r=reconcileMeteoraDlmmCashFlowProfitability({
   position:'position-1',currency:'USDC',informationCutoff:'2026-09-01T01:00:00Z',positionClosed:true,transactionHistoryComplete:true,
   flows:[valuedFlow('deposit','DEPOSIT',100000n),valuedFlow('withdraw','WITHDRAWAL',108000n)],
   benchmark:{benchmarkId:'bench-no-state',position:'position-1',currency:'USDC',hodlValueMinor:115000n,lpTerminalValueMinor:112000n,methodology:'MATCHED_HODL_COUNTERFACTUAL',observedAt:'2026-09-01T00:00:04Z',availableAt:'2026-09-01T00:00:05Z',evidenceIds:['benchmark:no-state']},
  })
  expect(r.impermanentLossMinor).toBeNull()
  expect(r.impermanentLossStatus).toBe('BENCHMARK_REQUIRED')
 })
 ...
 it('fails closed on malformed or mismatched benchmark evidence',()=>{
  expect(()=>reconcileMeteoraDlmmCashFlowProfitability({
   ...
   benchmark:{benchmarkId:'bench-bad',position:'other',currency:'USDC',hodlValueMinor:1n,lpTerminalValueMinor:1n,methodology:'MATCHED_HODL_COUNTERFACTUAL',observedAt:'2026-09-01T00:00:04Z',availableAt:'2026-09-01T00:00:05Z',evidenceIds:['benchmark:bad']},
  })).toThrow('benchmark_identity_mismatch')

  expect(()=>reconcileMeteoraDlmmCashFlowProfitability({
   position:'position-1',currency:'USDC',informationCutoff:'2026-09-01T01:00:00Z',positionClosed:true,transactionHistoryComplete:true,
   flows:[valuedFlow('deposit','DEPOSIT',100000n)],
   benchmark:{benchmarkId:'bench-method',position:'position-1',currency:'USDC',hodlValueMinor:1n,lpTerminalValueMinor:1n,methodology:'UNMATCHED' as any,observedAt:'2026-09-01T00:00:04Z',availableAt:'2026-09-01T00:00:05Z',evidenceIds:['benchmark:method']},
  })).toThrow('benchmark_methodology_invalid')
 })
