...
/** Orchestrates one authenticated user's playback without owning queue or provider logic. */
export class PlaybackSession {
  private state: PlaybackSessionState;
  constructor(
    private readonly userId: string,
    private readonly resolver: AuthorizedPlaybackResolver,
    private readonly host: PlaybackHost,
    initial: MusicControllerState,
  ) {
  ...
  async load(track: Track): Promise<UserPlaybackResult> {
    ...
    if (!result) {
      this.state = { ...this.state, playing: false, error: "No authorized playable source" };
