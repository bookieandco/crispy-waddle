...
export async function POST(request: Request) {
  ...
  let cast: CharacterCastRecord;
  try {
    cast = await loadDirectorCastRecord(privileged, { projectId, characterId });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'DIRECTOR_CAST_READ_FAILED';
    const status = message.includes('DIRECTOR_CAST_NOT_FOUND') ? 404 : 409;
