...
export class SupabaseMoneyResearchPersistence implements MoneyResearchPersistence, MoneyResearchTaskPersistence, MoneyResearchBatchPersistence {
  ...
  async saveResearchCase(input: PlannedResearchCase): Promise<PersistedResearchCase> {
    ...
    const { data, error } = await this.supabase.from("money_research_cases").upsert({
    ...
    }, { onConflict: "opportunity_id" }).select("id, opportunity_id").single()
    if (error || !data?.id || !data?.opportunity_id) throw new Error(error?.message || "Research case upsert did not return an id")

    for (const branch of input.branches) {
      await this.upsertResearchTask({ researchCaseId: String(data.id), branch, priority: researchTaskPriority(branch.status) })
    }
    return { id: String(data.id), opportunityId: String(data.opportunity_id) }
  }

  async upsertResearchTask(input: { researchCaseId: string; branch: PlannedResearchCase["branches"][number]; priority: number }): Promise<{ id: string }> {
    const existing = await this.supabase.from("money_research_tasks").select("id, status, priority").eq("research_case_id", input.researchCaseId).eq("branch", input.branch.kind).maybeSingle()
    if (existing.error) throw new Error(existing.error.message)
    const existingStatus = existing.data?.status as ResearchBranchStatus | undefined
    const state = preserveResearchTaskState({ existingStatus, plannedStatus: input.branch.status })
    const { data, error } = await this.supabase.from("money_research_tasks").upsert({
      research_case_id: input.researchCaseId,
      branch: input.branch.kind,
      question: input.branch.question,
      status: state.status,
      priority: state.priority,
    }, { onConflict: "research_case_id,branch" }).select("id").single()
