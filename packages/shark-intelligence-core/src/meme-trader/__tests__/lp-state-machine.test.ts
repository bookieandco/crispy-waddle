...
function u64(value: bigint): number[] { const bytes = []; for (let i = 0; i < 8; i++) { bytes.push(Number(value & 255n)); value >>= 8n }; return bytes }
...
describe('Solana LP state machine', () => {
  ...
  it('uses compiled Token-2022 burn data when parsed CPI is unavailable', () => {
    const data = base58([8, ...u64(500n)])
    const result = new SolanaLPStateMachine().decode(tx({ preTokenBalances: [{ accountIndex: 0, mint: 'LP', owner: 'OWNER', uiTokenAmount: { amount: '500', decimals: 6 } }], postTokenBalances: [{ accountIndex: 0, mint: 'LP', owner: 'OWNER', uiTokenAmount: { amount: '0', decimals: 6 } }], innerInstructions: [] }, [{ programId: TOKEN_2022, accounts: [0, 3], data }]), pool, 'LP')
    expect(result[0]).toMatchObject({ kind: 'BURN', tokenAccount: 'LP_ACCOUNT', amountRaw: 500n, confidence: 1 })
  })
  it('uses compiled Token-2022 transfer data', () => {
    const data = base58([3, ...u64(600n)])
    const result = new SolanaLPStateMachine().decode(tx({ preTokenBalances: [{ accountIndex: 0, mint: 'LP', owner: 'OWNER', uiTokenAmount: { amount: '1000', decimals: 6 } }], postTokenBalances: [{ accountIndex: 0, mint: 'LP', owner: 'OWNER', uiTokenAmount: { amount: '400', decimals: 6 } }], innerInstructions: [] }, [{ programId: TOKEN_2022, accounts: [0, 2], data }]), pool, 'LP')
