-- Retries of the same audit event must return the original durable row rather
-- than consume another chain position. A reused event id with different
-- content is rejected so idempotency cannot become an overwrite primitive.
