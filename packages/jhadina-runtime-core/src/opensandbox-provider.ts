...
function assertAttestation(handle: OpenSandboxHandle, executionId: string, requireSecureRuntime: boolean): SandboxIsolationEvidence {
  ...
  if (attestation.isolation.provider !== 'opensandbox') throw new Error('OpenSandbox attestation provider identity mismatch');
...
export class OpenSandboxProvider implements SandboxProviderPort {
  ...
  async provision(request: ExecutionRequest): Promise<OpenSandboxLease> {
    ...
    try {
      ...
      return {
        ...
        isolation,
        release: async () => { if (released) return; released = true; await this.client.deleteSandbox(handle.sandboxId); },
      };
    } catch (error) {
  ...
  async execute(request: ExecutionRequest, sandbox: SandboxLease): Promise<ExecutionResult> {
    ...
    if (bound.isolation.provider !== 'opensandbox') throw new Error('OpenSandbox lease provider identity mismatch');
