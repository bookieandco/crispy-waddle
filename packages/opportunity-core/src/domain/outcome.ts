...
const OUTCOME_SOURCE_OWNERS = new Set<OpportunityOutcomeSourceOwner>(['money_core','commerce','placement','overageos','user'])
...
export function calculateOpportunityOutcome(input: OpportunityOutcomeObservationInput): OpportunityOutcome {
  ...
  if (input.result !== 'won' && input.result !== 'lost') throw new Error('outcome result must be won or lost')
  ...
  if (!OUTCOME_SOURCE_OWNERS.has(input.sourceOwner)) throw new Error('outcome sourceOwner is invalid')
  if (!Array.isArray(input.evidenceRefs) || input.evidenceRefs.length === 0 || input.evidenceRefs.some((ref) => typeof ref !== 'string' || !ref.trim())) throw new Error('realized outcome requires non-empty evidence references')
  if (input.transactionRefs !== undefined && (!Array.isArray(input.transactionRefs) || input.transactionRefs.some((ref) => typeof ref !== 'string' || !ref.trim()))) throw new Error('transactionRefs must be an array of non-empty references')
...
export function applyOpportunityOutcome(
  ...
  if (outcome.opportunityId !== opportunity.id) throw new Error('Outcome does not belong to opportunity')
  if (!['ready', 'pursuing', 'won', 'lost'].includes(opportunity.status)) {
