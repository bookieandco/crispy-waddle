...
export class ControllerInputGate {
  ...
  authorize(event:InputIntegrityEvent,nowMs=Date.now()):ControllerInputGateResult{
    ...
    if(!Object.prototype.hasOwnProperty.call(CONTROLLER_INPUT_CAPABILITY,event.inputKind))return{allowed:false,reason:'controller-capability-mismatch',deviceId,sessionId};
    let binding;
