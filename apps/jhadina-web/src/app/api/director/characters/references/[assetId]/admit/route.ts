...
export async function POST(
  ...
  if (!asset) return NextResponse.json({ ok: false, error: 'DIRECTOR_CHARACTER_REFERENCE_NOT_FOUND' }, { status: 404 });
  ...
  if (asset.admission_status === 'rejected') {
    return NextResponse.json({ ok: false, error: 'DIRECTOR_CHARACTER_REFERENCE_REJECTED' }, { status: 409 });
  }
  ...
  if (!result.clean || !result.safe) {
    ...
    return NextResponse.json({
      ok: false,
      error: 'DIRECTOR_CHARACTER_REFERENCE_SCAN_REJECTED',
