...
export function createMediaPlayerStore(initial?: Partial<MediaSessionSnapshot>): MediaPlayerStore {
  ...
  return {
    ...
    play: () => requireSession().play(), pause: () => requireSession().pause(), seek: (position) => requireSession().seek(Math.max(0, position)), setVolume: (value) => requireSession().setVolume(Math.max(0, Math.min(1, value))), discoverTargets: () => requireSession().discoverTargets(), transfer: (target) => requireSession().transfer(target), disconnect: () => requireSession().disconnect(),
  };
}
