...
function hashRecord(input: Omit<ApprovalAuditRecord, "recordHash">): string {
  // Deterministic integrity marker. Cryptographic persistence can be layered
  // underneath the ledger without changing the state-machine contract.
  let hash = 2166136261;
...
function transition(
  ...
  if (!allowed) {
    throw new Error(`Invalid approval transition: ${state.status} -> ${to}`);
  }
  ...
  const audit: ApprovalAuditRecord = { ...unsigned, recordHash: hashRecord(unsigned) };

  return {
    status: to,
    executionEligible: to === "EXECUTION_ELIGIBLE",
    audit,
  };
}
...
export function approvePendingBooking(
  state: ApprovalState,
  authorization: AuthorizationContext,
  now: string,
): ApprovalState {
  const result = transition(state, "APPROVED", "APPROVE", authorization, now);
  return { ...state, status: result.status, audit: [...state.audit, result.audit] };
}

export function markBookingExecutionEligible(
  state: ApprovalState,
  authorization: AuthorizationContext,
  now: string,
): ApprovalState {
  const result = transition(state, "EXECUTION_ELIGIBLE", "MARK_EXECUTION_ELIGIBLE", authorization, now);
  return { ...state, status: result.status, audit: [...state.audit, result.audit] };
}
