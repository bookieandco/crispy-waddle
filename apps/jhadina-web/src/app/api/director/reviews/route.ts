...
import { commitDirectorMediaReview } from '@jhadina/director-core';
...
export async function POST(req:Request){
 ...
 try{const rt=await createConfiguredDirectorGenerationRuntime(client);const authority=await rt.reviewAuthority.resolve(raw);const decision=await commitDirectorMediaReview({authority,decision:raw.decision,decisionId:raw.decisionId,decidedBy:raw.decidedBy,note:raw.note,repository:rt.reviewRepository});return NextResponse.json({ok:true,decision},{status:201});}
