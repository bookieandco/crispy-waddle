...
export interface MediaPlayerStore { getState(): MediaSessionSnapshot; subscribe(listener: (state: MediaSessionSnapshot) => void): () => void; attachSession(session: UnifiedMediaSession): () => void; load(item: MediaItem): void; restore(item: MediaItem): Promise<MediaPlaybackProgress | null>; persistProgress(): Promise<MediaPlaybackProgress | null>; setQueue(items: MediaItem[], currentIndex?: number): void; enqueue(item: MediaItem): void; removeFromQueue(index: number): void; clearQueue(): void; playAt(index: number): Promise<void>; next(): Promise<void>; previous(): Promise<void>; setShuffle(enabled: boolean): void; setRepeat(mode: MediaRepeatMode): void; play(): Promise<void>; pause(): Promise<void>; seek(positionSeconds: number): Promise<void>; setVolume(value: number): Promise<void>; discoverTargets(): Promise<PlaybackTarget[]>; transfer(target: PlaybackTarget): Promise<void>; disconnect(): Promise<void>; }
export interface MediaPlayerStoreOptions { userId?: string; persistence?: MediaPlaybackProgressRepository; resumeThresholdMs?: number; completionThreshold?: number; }
...
export function createMediaPlayerStore(initial?: Partial<MediaSessionSnapshot>, options: MediaPlayerStoreOptions = {}): MediaPlayerStore {
 ...
 let session: UnifiedMediaSession | undefined; let unsubscribeSession: (() => void) | undefined; let persistTimer: ReturnType<typeof setTimeout> | undefined;
 ...
 const toProgress = (): MediaPlaybackProgress | null => { const key = currentKey(); if (!key || !options.userId) return null; const duration = state.playback.durationMs ?? state.item?.durationMs; return { userId: options.userId, providerId: key.providerId, itemId: key.itemId, positionMs: Math.max(0, state.playback.positionMs), durationMs: duration, completed: state.playback.status === 'ended' || Boolean(duration && duration > 0 && state.playback.positionMs / duration >= completionThreshold), updatedAt: new Date().toISOString() }; };
 const persist = async () => { const progress = toProgress(); if (!progress || !options.persistence) return null; return options.persistence.upsert(progress); };
 return {
  getState: () => state,
  subscribe(listener) { listeners.add(listener); return () => listeners.delete(listener); },
  attachSession(nextSession) { unsubscribeSession?.(); session = nextSession; publish(nextSession.getState()); unsubscribeSession = nextSession.subscribe((next) => { publish(next); if (next.playback.status === 'ended') void persist(); }); return () => { if (session === nextSession) { unsubscribeSession?.(); unsubscribeSession = undefined; session = undefined; } }; },
  ...
  async persistProgress() { if (persistTimer) { clearTimeout(persistTimer); persistTimer = undefined; } return persist(); },
  ...
  setShuffle(enabled) { publish({ ...state, queue: { ...state.queue, shuffle: enabled } }); }, setRepeat(mode) { publish({ ...state, queue: { ...state.queue, repeat: mode } }); },
  play: () => requireSession().play(),
  pause: async () => { await requireSession().pause(); await persist(); },
  seek: async (position) => { await requireSession().seek(Math.max(0, position)); await persist(); },
  setVolume: (value) => requireSession().setVolume(Math.max(0, Math.min(1, value))),
  discoverTargets: () => requireSession().discoverTargets(),
  transfer: async (target) => { await requireSession().transfer(target); await persist(); },
  disconnect: async () => { await requireSession().disconnect(); await persist(); },
 };
}
