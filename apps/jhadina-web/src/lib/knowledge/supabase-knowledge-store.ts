...
export function createSupabaseKnowledgeStore(input?:SupabaseClient|null):KnowledgeStore{
 ...
 const graph=async(q:KnowledgeQuery)=>{
   if(!q.entityIds?.length)return []
   const {data,error}=await s.rpc("jhadina_traverse_knowledge_graph",{p_seed_node_ids:q.entityIds,p_owner_id:q.ownerId??null,p_scope:q.scope??null,p_as_of:q.asOf??new Date().toISOString(),p_max_depth:2,p_limit:100})
   if(error)throw new Error(`Knowledge graph traversal failed: ${error.message}`)
   const labels=[...new Set((data??[]).map((x:any)=>x.label).filter(Boolean))]
   if(!labels.length)return []
   const batches=await Promise.all(labels.slice(0,25).map(label=>run({...q,text:label,mode:"exact",limit:4})))
   return batches.flat()
 }
 return {exact:run,semantic:run,graph}
}
