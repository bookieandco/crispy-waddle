...
let persistentMediaElementOwnerToken = 0;
let nextPersistentMediaElementOwnerToken = 0;
...
/** Mount and atomically claim ownership of the shared media element. */
export function mountPersistentMediaElement(host: HTMLElement): HTMLVideoElement {
  ...
  persistentMediaElementOwnerToken = ++nextPersistentMediaElementOwnerToken;
  if (video.parentElement !== host) host.appendChild(video);
  video.style.display = '';
  return video;
}

/** Return the ownership token created by the most recent mount. */
export function getPersistentMediaElementOwnerToken(): number | null {
  return persistentMediaElementOwner ? persistentMediaElementOwnerToken : null;
}

/**
 * Release only the mount that the caller actually created. The token is
 * essential when React runs an older effect cleanup before the DOM host has
 * been removed; parentElement alone cannot distinguish stale same-host cleanup.
 */
export function releasePersistentMediaElement(host?: HTMLElement, ownerToken?: number): void {
  if (!persistentMediaElement) return;
  const owner = persistentMediaElementOwner;
  if (!owner) return;
  if (host !== owner) return;
  if (ownerToken !== undefined && ownerToken !== persistentMediaElementOwnerToken) return;
  if (ownerToken === undefined && owner.isConnected) return;
  ...
  persistentMediaElementOwnerToken = 0;
}
...
export async function ensureMediaPlaybackSession(config: UnifiedMediaSessionConfig, item: MediaQueueItem, request?: MediaPlaybackLoadRequest): Promise<UnifiedMediaSession> { const load = request ? { request, generation: ++latestLoadRequestGeneration } : createLoadRequest(); assertLoadRequestCurrent(load.request, load.generation); if (!session) { if (!sessionInitialization) { sessionInitialization = Promise.resolve().then(() => { if (session) return session; const created = createUnifiedMediaSession(config); session = created; queueItem(item); observeSession(created); return created; }).finally(() => { sessionInitialization = null; }); } const initialized = await sessionInitialization; assertLoadRequestCurrent(load.request, load.generation); if (!session || session !== initialized) throw new Error('JHADINA_MEDIA_PLAYBACK_SESSION_INITIALIZATION_LOST'); } const sharedSession = session; return enqueueSessionCommand(async () => { assertLoadRequestCurrent(load.request, load.generation); if (!session || session !== sharedSession) throw new Error('JHADINA_MEDIA_PLAYBACK_SESSION_OWNERSHIP_LOST'); const current = getMediaPlaybackStore().getState().current; if (samePlaybackItem(current, item)) return sharedSession; await progressPersistence?.flush(false); progressPersistence?.dispose(); progressPersistence = null; progressPersistenceConfig = null; unsubscribeSession?.(); unsubscribeSession = null; const previousSession = session; sessionGeneration += 1; session = null; previousSession.dispose(); assertLoadRequestCurrent(load.request, load.generation); const replacement = createUnifiedMediaSession(config); session = replacement; queueItem(item); observeSession(replacement); publishSnapshot(); return replacement; }, load.request, load.generation); }
