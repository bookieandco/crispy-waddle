...
export function PersonalCommandFeed() {
  ...
  const filters: Array<{ id: typeof filter; label: string }> = [
    { id: 'all', label: 'All' }, { id: 'work', label: 'Work' }, { id: 'opportunity', label: 'Opportunities' },
  ...
  return (
    <section style={{ maxWidth: 820, margin: '0 auto', padding: '48px 20px 96px' }}>
      ...
      <div style={{ display: 'grid', gap: 14 }}>{feed.map((item) => <article key={item.id} style={{ border: '1px solid rgba(255,255,255,.09)', borderRadius: 24, padding: 22, background: 'rgba(255,255,255,.035)', boxShadow: '0 14px 50px rgba(0,0,0,.18)' }}><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><div style={{ width: 42, height: 42, borderRadius: 13, display: 'grid', placeItems: 'center', background: 'rgba(255,255,255,.08)', fontSize: 18 }}>{item.platform ? glyph[item.platform] : glyph[item.kind]}</div><div><div style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '.2em', opacity: .45 }}>{item.label}</div>{item.status && <div style={{ fontSize: 11, opacity: .35, marginTop: 3 }}>{item.status.replaceAll('_', ' ')}</div>}</div></div><h3 style={{ margin: '18px 0 8px', fontSize: 21 }}>{item.title}</h3><p style={{ margin: 0, lineHeight: 1.6, opacity: .62, whiteSpace: 'pre-wrap' }}>{item.body}</p>{item.action && item.href && <Link href={item.href} style={{ marginTop: 18, display: 'inline-block', border: '1px solid rgba(255,255,255,.12)', borderRadius: 12, padding: '9px 14px', background: 'rgba(255,255,255,.06)', color: 'inherit', textDecoration: 'none' }}>{item.action}</Link>}</article>)}{items.length === 0 && <div style={{ padding: 24, borderRadius: 20, border: '1px dashed rgba(255,255,255,.12)', opacity: .5 }}>Connect your social accounts and Jhadina will start mixing their activity with your work and opportunities here.</div>}</div>
    </section>
  );
}
