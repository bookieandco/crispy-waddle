...
const toDeliveryRow = (r: AlertDeliveryRecord) => ({ id: r.id, alert_id: r.alertId, recipient_id: r.recipientId, channel: r.channel, priority: r.priority, status: r.status, payload: r.payload ?? {}, idempotency_key: r.idempotencyKey, attempt: r.attempt, created_at: r.createdAt, updated_at: r.updatedAt })
const toDelivery = (r: any): AlertDeliveryRecord => ({ id: r.id, alertId: r.alert_id, recipientId: r.recipient_id, channel: r.channel, priority: r.priority, status: r.status, payload: r.payload ?? {}, idempotencyKey: r.idempotency_key, attempt: r.attempt, createdAt: r.created_at, updatedAt: r.updated_at })
