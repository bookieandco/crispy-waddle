...
**Branch:** `fix/intcom-final-hardening` (PR #349, merged `a18b9147759e93dfd8bea4bf949fcdc6880af5ca`); targeted CI added by PR #351 (merged `ffa56d29ec11f053c098a1f2104be2b4c834faf7`).
**Objective:** Transport-neutral communications intent -> identity/policy/approval -> ActionExecutor -> transport adapter -> delivery evidence. Reticulum is an adapter, not the core architecture or an authority.
**Dependencies:** JH-001, JH-002, JH-012, JH-042. JH-012 remains separately ACTIVE for its own observation/workspace verification debt; explicit user direction allowed the communications slice to proceed without treating that debt as JH-013 authority.
...
**Definition of Done:** Met. Recipient/device and transport identities remain separate; trust ends at `trusted` and `communications.send` authorization is an independent evidence-backed grant; communication intent crosses canonical policy/approval; the actual Reticulum send now executes inside the `ActionExecutor` handler; routing is capability/health/adapter constrained; delivery receipts preserve recipient/intent/correlation/transport lineage and sanitize failure codes; inbound observations use atomic replay claims and persist rejection evidence without granting trust/authorization; store-forward uses a repository durability boundary, dispatch-level idempotency and evidence-bearing reconciliation without manufacturing new authorization.

**Final audit/repair (2026-09-20):**
- Repaired `DISCOVERY != TRUST != AUTHORIZATION`; trust state can no longer encode authorization.
- Repaired dispatch correlation validation at the routing boundary and adapter-constrained route selection.
- Repaired the execution-boundary defect where transport send occurred after `ActionExecutor`; Reticulum transmission is now the governed handler side effect. Policy denial and approval-required outcomes produce zero sends.
- Repaired inbound replay TOCTOU by replacing `has()+add()` with atomic `claim()`; concurrent duplicate ingestion permits one winner, replay/auth failures become canonical evidence, and a failed evidence append releases the claim for safe retry.
- Repaired store-forward idempotency so duplicate dispatch lineage is suppressed even under a different queue item ID; repository state survives queue reconstruction; successful forwarding is reconciled before secondary evidence recording so an evidence outage cannot duplicate the send; failure evidence exposes only `FORWARD_FAILED`, not provider error text.
- Repaired delivery evidence with recipient endpoint lineage, canonical receipt validation, and bounded failure-code format; Reticulum receipts pass through the same validator.
- Added a dedicated `INTCOM Core` CI gate so this slice can be verified independently while unrelated workspace audit debt remains visible.

**Verification (real GitHub Actions, 2026-09-20):**
- `INTCOM Core` run 35515165557: SUCCESS.
- `pnpm --filter @jhadina/intelligence-core type-check`: SUCCESS.
- `pnpm --filter @jhadina/intelligence-core test`: 49/49 passing, 0 failures.
- Spatial Conformance run 35515165673: SUCCESS.
- Repo-wide Launch Gate run 35515165464 remains red in another subsystem (`@jhadina/director-core#type-check`); `@jhadina/intelligence-core` was not the failing package. This is retained as separate workspace audit debt rather than misreported as an INTCOM failure.

**Architectural impact:** The governed communications chain is now `CommunicationIntent -> explicit recipient authorization evidence -> policy/approval -> ActionExecutor -> transport handler -> route/Reticulum -> DeliveryReceipt -> durable evidence`. Offline reconciliation may retry only the same already-authorized dispatch and cannot mint authority. Inbound transport data terminates as evidence and never becomes a direct execution path. Homebase/device/cloud can implement the repository boundary without becoming a second policy authority.

**Next Step:** None for JH-013 — complete. Continue JH-012 and the canonical queue independently.
...
**Priority:** P2
**Status:** DONE
**Branch:** `audit/money-final-complete` (PR #486)
...
**Implementation (2026-09-20):** Added durable RLS-protected Plaid Item/account authorization metadata; authenticated ownership resolution; account output scoping and transaction ownership enforcement; governed transaction intelligence for recurring/duplicate review signals; command-center transaction consumption; private service-role-only bank credentials; and adversarial acceptance tests for owned reads, unknown/cross-user account denial before provider I/O, empty/revoked ownership, identity binding and durable audit evidence. Payment/transfer/withdrawal capabilities remain outside this spine.
**MONEY-FINAL completion (2026-09-20):** The final repository/handoff audit closed the remaining software gaps. It reconciled Money with Action Core, SHARK, Sports, Capital Lab, stock/FX, reference provenance and live Swlc; applied `20260920223551_money_final_least_privilege_closure`; removed stale authenticated bank mutation policies; revoked latent anon/authenticated grants from backend Money research tables; made Money CI watch Money migrations; and implemented MONEY-METALS-01/02 for XAU/XAG/XPT/XPD point-in-time reality plus research-only intelligence; and added MONEY-PREDICTION-01/02 so prediction markets are a first-class provider-independent Money research domain instead of only an enum/calibration/reference concept.
**Verification:** PR #486 code head `5dd0a344ae1771c9f0f5058f52fce26222a088f3`: Money R13B #233 SUCCESS; SHARK #124 SUCCESS; Spatial #792 SUCCESS; Evolution #1283 SUCCESS; Staffing #2032 SUCCESS; Director #199 SUCCESS. Live Swlc post-migration grants verify authenticated SELECT-only bank ownership metadata and service-role-only credentials/research workflow state.
**Closure receipt:** `docs/architecture/MONEY-FINAL-CLOSURE-RECEIPT-2026-09-20.md`.
**Production banking admission:** NOT GRANTED. A fresh Vercel deployment is currently blocked by the account deployment build-rate quota; required server-only deployment secrets therefore remain unverified; GitHub history has no PL-8 live Plaid dispatch; and the full authenticated Link -> private credential persistence -> owned account read -> governed transaction read -> durable audit -> disconnect -> post-disconnect denial drill has not run. These are commissioning gates, not reopened repository defects. Until those receipts exist, Money must not be represented as production-Plaid accepted.
**Authority boundary:** This DONE status closes the Money repository/software program only. It does not authorize autonomous stock/FX/metals trading, SHARK protected-fund access, automatic sports betting, prediction-market auto-execution, or financial mutation outside Action Core. Prediction-market provider adapters and executors remain separate admission work.
