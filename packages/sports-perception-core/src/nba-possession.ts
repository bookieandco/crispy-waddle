...
export function resolveNBAPossession(state: NBAPossessionState, rng: RandomSource): NBAPossessionResult {
  ...
  const turnoverProbability = clamp(0.12 - state.ballHandler.ballSecurity / 1000 + turnoverAdjustment / -1000);
  const foulProbability = clamp(0.08 + Math.max(0, -(matchup.attributeAdjustments.finishing ?? 0)) / 1000);
  const driveProbability = clamp(0.32 + state.ballHandler.shotCreation / 500 + (matchup.attributeAdjustments.finishing ?? 0) / 500);
  const threeProbability = clamp(0.35 + state.ballHandler.threePoint / 300);
  ...
  if (action === 'DRIVE' || action === 'SHOT_2' || action === 'SHOT_3') {
    const attribute = action === 'SHOT_3' ? 'shot_creation' : action === 'DRIVE' ? 'finishing' : 'shot_creation';
    const baseProbability = action === 'SHOT_3' ? 0.36 : action === 'DRIVE' ? 0.56 : 0.50;
    const contest = resolvePlayerEventContest({
      eventId: state.possessionId,
      outcome: action,
      attribute,
      baseProbability,
      matchup,
    });
    const made = rng.next() < contest.resolvedProbability;
    if (made) points = action === 'SHOT_3' ? 3 : 2;
  }
  ...
  return Object.freeze({
    ...
    action,
    points,
    nextOffenseTeamId: state.defenseTeamId,
    clockSecondsRemaining: Math.max(0, state.clockSeconds - elapsed),
    probabilities: Object.freeze({ turnover: turnoverProbability, foul: foulProbability, drive: driveProbability, three: threeProbability }),
    evidenceIds: matchup.attacker.evidenceIds.concat(matchup.defender.evidenceIds).filter((id, i, ids) => ids.indexOf(id) === i).sort(),
  });
}
