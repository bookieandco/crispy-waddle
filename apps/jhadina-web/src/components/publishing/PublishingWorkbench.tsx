...
export function PublishingWorkbench() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.layout}>
      <aside className={styles.sidebar}><h2>Workbench</h2>{tools.map((tool) => <button type="button" key={tool} className={styles.tool} onClick={() => setTab(tool === 'Editor' || tool === 'Visuals' ? 'Write' : tool === 'Reader' || tool === 'Formats' || tool === 'Proof' ? 'Preview' : tool === 'KDP' || tool === 'Store' || tool === 'POD' || tool === 'Analytics' ? 'Publish' : tool === 'Research' ? 'Research' : 'Publish')}>{tool}<span>›</span></button>)}<div className={styles.notice}><b>✦ Jhadina noticed</b><span>{saved ? 'Draft saved. Your next publishing step can be surfaced here.' : 'Saving your changes…'}</span></div></aside>
