...
export type PoolCandidate = {
  ...
  source: string
  evidenceId: string
  metadata?: { accounts?: PoolAccountMetadata[] }
}
...
export class DexScreenerPoolDiscoverySource implements PoolDiscoverySource {
  ...
  async discoverPools(launch: TokenLaunch): Promise<PoolCandidate[]> {
    ...
    const body: unknown = await response.json(); if (!Array.isArray(body)) return []
    ...
    return body.flatMap((raw, index) => {
      ...
      const dexId = stringValue(pair.dexId)
      const programId = dexId === 'raydium' ? '675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8' : dexId === 'pumpswap' ? 'pAMMBay6oceH9fJKBRHGP5D4bD4sWpmSwMn52FMfXEA' : undefined
      const liquidity = pair.liquidity && typeof pair.liquidity === 'object' ? pair.liquidity as Record<string, unknown> : undefined
      return [{ poolAddress, dexId, programId, baseMint: stringValue(baseToken?.address), quoteMint: stringValue(quoteToken?.address), liquidityUsd: finite(liquidity?.usd) ? liquidity.usd : undefined, observedAt, source: 'dexscreener-pool-discovery', evidenceId: `dexscreener:pool:${launch.tokenAddress}:${poolAddress}:${observedAt}:${index}` }]
    })
  }
}
