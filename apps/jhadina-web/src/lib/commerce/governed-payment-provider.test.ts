...
describe("GovernedPaymentProvider — explicit capability boundary, authorization before provider call, full audit trail", () => {
  test("charge is an allowed capability: authorized before the provider call, recorded started->completed", async () => {
    const governed = new GovernedPaymentProvider(provider())
    const intent = await governed.createPaymentIntent(paymentRequest())
    expect(intent.status).toBe("captured")

    const trail = governed.getAuditTrail().filter((e) => e.actionId === "pay_1")
    expect(trail.map((e) => e.status)).toEqual(["started", "completed"])
    expect(trail.every((e) => e.type === "commerce.payment.charge")).toBe(true)
    expect(trail.every((e) => e.userId === "cust_1")).toBe(true)
  })

  test("payout is denied by policy: the wrapped provider is never called, and the denial itself is audited", async () => {
    ...
    const governed = new GovernedPaymentProvider(underlying)
    ...
    const trail = governed.getAuditTrail().filter((e) => e.actionId === "po_1")
  ...
  test("reconcile is denied the same way", async () => {
    ...
    const trail = governed.getAuditTrail().filter((e) => e.type === "commerce.payment.reconcile")
    expect(trail).toHaveLength(1)
    expect(trail[0].status).toBe("denied")
  })
  ...
  test("a failed charge is audited as failed, not silently dropped", async () => {
    ...
    const governed = new GovernedPaymentProvider(failing)

    await expect(governed.createPaymentIntent(paymentRequest({ paymentId: "pay_declined" }))).rejects.toThrow()

    const trail = governed.getAuditTrail().filter((e) => e.actionId === "pay_declined")
    expect(trail.map((e) => e.status)).toEqual(["started", "failed"])
    expect(trail[1].metadata?.reason).toBeDefined()
  })

  test("refund is an allowed capability and composes through the same governed gate", async () => {
    const governed = new GovernedPaymentProvider(provider())
    await governed.createPaymentIntent(paymentRequest({ paymentId: "pay_refund_me" }))
    const refunded = await governed.refund({ refundId: "re_1", paymentId: "pay_refund_me", reason: "customer_request", requestedBy: "cust_1" })
    expect(refunded.status).toBe("refunded")

    const trail = governed.getAuditTrail().filter((e) => e.actionId === "re_1")
    expect(trail.map((e) => e.status)).toEqual(["started", "completed"])
