-- A5-R8: provider operation identity must be distinct from payment_id.
-- A single payment can legitimately have multiple governed irreversible operations
-- (for example charge then capture, or a later refund). The durable key is the
-- provider operation id; payment_id remains the external provider/idempotency id.
...
CREATE INDEX IF NOT EXISTS idx_jhadina_commerce_payment_operation_payment
  ON jhadina_commerce_payment_operation (provider, payment_id, created_at DESC);
...
BEGIN
  ...
  VALUES
    (p_provider, p_operation_id, p_payment_id, p_actor_id, p_action_id, p_capability, p_request_fingerprint, 'processing')
  ON CONFLICT (provider, operation_id) DO NOTHING;

  GET DIAGNOSTICS inserted = ROW_COUNT;
  SELECT * INTO r
  FROM jhadina_commerce_payment_operation
  WHERE provider = p_provider AND operation_id = p_operation_id;

  IF r.payment_id <> p_payment_id
     OR r.actor_id <> p_actor_id
     OR r.action_id <> p_action_id
     OR r.capability <> p_capability
