...
describe('wallet cluster threshold sensitivity',()=>{
 it('reports threshold sensitivity without selecting a production threshold',()=>{
  ...
  expect(r.rows.find(x=>x.thresholdId==='loose')?.falsePositiveRate).toBe(.5)
  expect(r.rows.find(x=>x.thresholdId==='loose')?.falseNegativeRate).toBe(0)
  expect(r.rows.find(x=>x.thresholdId==='strict')?.falsePositiveRate).toBe(1)
  expect(r.rows.find(x=>x.thresholdId==='strict')?.falseNegativeRate).toBe(1)
  expect(r.bestThresholdId).toBeNull()
  expect(r.selectionStatus).toBe('SENSITIVITY_ONLY')
