...
import { normalizeYouTubeMusicImport } from "@jhadina/music-core";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const artist of imported.artists) await musicRepository.upsertArtist(userId, artist);
    for (const track of imported.tracks) await musicRepository.upsertTrack(userId, track);
    await musicRepository.upsertSource({ id: imported.sourceId, userId, kind: "youtube_music", name: "YouTube Music", authorized: true, metadata: { importedAt: new Date().toISOString() } });
