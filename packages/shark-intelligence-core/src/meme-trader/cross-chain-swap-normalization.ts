...
const decimalAmount=(raw:string,decimals:number):number=>{
  if(!/^[0-9]+$/.test(raw)||!Number.isInteger(decimals)||decimals<0||decimals>30)throw new Error('shark_cluster_amount_invalid')
  const value=BigInt(raw),base=10n**BigInt(decimals),whole=value/base,fraction=value%base
  const wholeNumber=Number(whole),fractionNumber=Number(fraction)/Number(base)
  if(!Number.isSafeInteger(wholeNumber))throw new Error('shark_cluster_amount_precision_unsafe')
  const normalized=wholeNumber+fractionNumber
  if(!Number.isFinite(normalized))throw new Error('shark_cluster_amount_precision_unsafe')
  return normalized
}
...
export function normalizeCrossChainSwapObservation(input:CrossChainSwapObservation):NormalizedClusterSwap{
  ...
  return Object.freeze({
    ...
    amountIn:decimalAmount(input.amountInRaw,input.tokenInDecimals),
    amountOut:decimalAmount(input.amountOutRaw,input.tokenOutDecimals),
