...
test('builds different immutable fundamental states at different information cutoffs', () => {
  const later = fact({ factId: 'fact-2', filingId: 'filing-2', concept: 'NetIncome', availableAt: '2026-03-01T12:00:00Z' }); const earlyState = buildFundamentalState('issuer-1', [fact(), later], '2026-02-25T00:00:00Z', '2026-03-02T00:00:00Z', '035.1', 'snapshot-early', [], 'state-hash-early'); const lateState = buildFundamentalState('issuer-1', [fact(), later], '2026-03-02T00:00:00Z', '2026-03-02T00:01:00Z', '035.1', 'snapshot-late', [], 'state-hash-late')
...
test('SHARK input cannot acquire a fact after the state cutoff', () => {
  const later = fact({ factId: 'fact-2', filingId: 'filing-2', availableAt: '2026-03-01T12:00:00Z' }); const state = buildFundamentalState('issuer-1', [fact(), later], '2026-02-25T00:00:00Z', '2026-03-02T00:00:00Z', '035.1', 'snapshot-early', [], 'state-hash-early'); const input = toSharkFundamentalInput(state, 'instrument-1', [fact(), later])
...
test('symbol changes do not create a new instrument identity', () => {
  const relationships = [relation({ relationshipId: 'symbol-old', relationshipType: 'ISSUER_OF', effectiveAt: '2026-01-01T00:00:00Z', expiresAt: '2026-06-01T00:00:00Z' }), relation({ relationshipId: 'symbol-new', relationshipType: 'ISSUER_OF', effectiveAt: '2026-06-01T00:00:00Z' })]
  assert.deepEqual(selectIssuerInstrumentRelationshipsAt(relationships, 'issuer-1', 'instrument-1', '2026-05-31T23:59:59Z').map((item) => item.relationshipId), ['symbol-old'])
  assert.deepEqual(selectIssuerInstrumentRelationshipsAt(relationships, 'issuer-1', 'instrument-1', '2026-06-01T00:00:00Z').map((item) => item.relationshipId), ['symbol-new'])
})

test('issuer successor can inherit a security relationship without rewriting historical issuer identity', () => {
  const successor = relation({ relationshipId: 'successor', issuerId: 'issuer-2', instrumentId: 'instrument-1', relationshipType: 'SUCCESSOR_SECURITY', effectiveAt: '2026-06-15T00:00:00Z' })
  assert.equal(relationshipWasEffectiveAt(successor, '2026-06-14T23:59:59Z'), false); assert.equal(relationshipWasEffectiveAt(successor, '2026-06-15T00:00:00Z'), true)
})
