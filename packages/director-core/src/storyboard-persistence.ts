...
export class SupabaseStoryboardRepository implements StoryboardBindingRepository {
  ...
  async getBoard(boardId: string, projectId: string): Promise<StoryboardBoard | null> {
    const { data, error } = await this.client.from('director_storyboard_boards')
      .select('id,sequence_id,project_id,shot_id,ordinal,status,title,description,script_ref,reference_asset_ids,continuity_anchor_ids,continuity_locks,camera_language,framing,action,notes,cinematography,camera_plan,performance_plan,realism_plan,version,artifact_ids,updated_at')
