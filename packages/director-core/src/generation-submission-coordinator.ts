...
export class GenerationSubmissionCoordinator {
  constructor(private readonly repository: GenerationRepository, private readonly workerId: string, private readonly leaseMs = 30_000) {}
  ...
  private startHeartbeat(submission: GenerationSubmissionOutbox): { stop: () => void; lost: () => boolean } {
    ...
    let lost = false;
    const timer = setInterval(() => { void this.repository.renewSubmissionLease(submission.id, this.workerId, submission.leaseToken!, this.leaseMs).then((renewed) => { if (!renewed) lost = true; }).catch(() => { lost = true; }); }, Math.max(10, Math.floor(this.leaseMs / 3)));
    return { stop: () => clearInterval(timer), lost: () => lost };
  }

  private async acknowledgeRecovered(task: GenerationTask, execution: GenerationExecution, submission: GenerationSubmissionOutbox, provider: GenerationProvider, recovered: GenerationResult): Promise<{ result?: GenerationResult; submission: GenerationSubmissionOutbox }> {
    if (!recovered.providerJobId) return { result: recovered, submission };
    const currentExecution = await this.repository.getExecution(execution.id);
    if (!currentExecution?.leaseToken || currentExecution.leaseOwner !== this.workerId) return { result: recovered, submission };
    const completedAt = new Date().toISOString();
    const acknowledged = await this.repository.acknowledgeSubmissionAndSaveState(submission.id, this.workerId, submission.leaseToken!, currentExecution.leaseToken, recovered.providerJobId, { ...task, status: recovered.status, error: recovered.error, updatedAt: completedAt }, { ...currentExecution, providerJobId: recovered.providerJobId, status: recovered.status, error: recovered.error, updatedAt: completedAt });
    if (!acknowledged) {
      const existing = await this.repository.getSubmissionByIdempotencyKey(provider.descriptor.id, task.idempotencyKey);
      return { result: recovered, submission: existing ?? submission };
    }
    return { result: recovered, submission: acknowledged.submission };
  }
  ...
  async submit(task: GenerationTask, execution: GenerationExecution, provider: GenerationProvider): Promise<{ result?: GenerationResult; submission: GenerationSubmissionOutbox }> {
    if (provider.submissionGuarantee === 'non-idempotent') throw new Error(`Provider ${provider.descriptor.id} is non-idempotent and cannot use the durable submission outbox`);
    ...
    try {
      ...
      // Reconcile an expired/crashed prior attempt before creating another provider side effect.
      if (provider.findByIdempotencyKey) {
        const recovered = await provider.findByIdempotencyKey(task.idempotencyKey);
        if (recovered?.providerJobId) return this.acknowledgeRecovered(task, execution, submission, provider, recovered);
      }
      ...
      const completedAt = new Date().toISOString();
      const acknowledged = await this.repository.acknowledgeSubmissionAndSaveState(submission.id, this.workerId, submission.leaseToken!, execution.leaseToken!, result.providerJobId, { ...task, status: result.status, error: result.error, updatedAt: completedAt }, { ...execution, providerJobId: result.providerJobId, status: result.status, error: result.error, updatedAt: completedAt });
      if (!acknowledged) {
        const existing = await this.repository.getSubmissionByIdempotencyKey(provider.descriptor.id, task.idempotencyKey);
      ...
      if (!heartbeat?.lost()) await this.repository.markSubmissionRecoveryRequired(submission.id, this.workerId, submission.leaseToken!, message).catch(() => undefined);
      throw error;
