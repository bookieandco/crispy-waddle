...
describe('actor intelligence graph and launch ingestion', () => {
  ...
  it('raises developer risk from a verified developer-owned LP withdrawal', () => {
    const base = {
      assessmentId: 'a2', assessedAt: '2026-01-01T00:00:00Z', token: { chainId: 'solana', tokenAddress: 'TokenA' }, tradeType: 'new-pair-speculation' as const,
      marketActivityQuality: { score: .8, volumeScore: .8, liquidityScore: .8, flowScore: .8, buyerGrowthScore: .8, manipulationPenalty: 0, reasons: [] }, supplyControl: { score: 0, deployerRisk: 0, concentrationRisk: 0, bundledSupplyRisk: 0, liquidityControlRisk: 0, reasons: [] }, holderCohort: { score: .8, profitableTrackedWallets: 0, accumulatingWallets: 0, distributingWallets: 0, reasons: [] }, attention: { score: .8, crossSourceConfirmation: .8, engagementQuality: .8, sourceCredibility: .8, manipulationPenalty: 0, reasons: [] }, strategyFit: { score: .8, matchedSignals: [], conflicts: [] }, riskAssessment: { marketIntegrity: .1, liquidityRisk: .1, supplyControlRisk: 0, holderConcentrationRisk: .1, walletCohortRisk: 0, socialManipulationRisk: 0, narrativeFragilityRisk: .1, developerRisk: 0, contractRisk: 0, networkRisk: .1, attentionQuality: .8, exitLiquidityRisk: .1, overallRisk: .1, band: 'candidate' as const }, thesis: 'test', invalidation: { conditions: ['liquidity loss'], severity: 'high' as const }, positionPlan: { maxPositionFraction: .01, entryConditions: [], profitTakingConditions: [], exitConditions: [] }, confidence: .5, evidenceIds: ['m2'], assessmentVersion: 'test'
    }
    const withdrawal: LPWithdrawalAttribution = { signature: 'SIG', observedAt: '2026-01-01T00:00:00Z', poolAddress: 'POOL', lpMint: 'LP', lpTokenAccount: 'LP_ACCOUNT', lpAmountRaw: 1000n, ownerBefore: 'DEVELOPER', lpStateEventId: 'lp-event', raydiumWithdrawalEventId: 'withdrawal-event', developerAssociation: 'MATCHED', evidenceIds: ['lp-withdrawal'], confidence: .9 }
    const enriched = applyActorRiskIntelligence(base, { verifiedLPWithdrawals: [withdrawal] })
    ...
    expect(enriched.evidenceIds).toContain('lp-withdrawal')
  })

  it('does not treat unmatched or low-confidence withdrawals as developer control', () => {
    const base = {
      assessmentId: 'a3', assessedAt: '2026-01-01T00:00:00Z', token: { chainId: 'solana', tokenAddress: 'TokenA' }, tradeType: 'new-pair-speculation' as const,
      marketActivityQuality: { score: .8, volumeScore: .8, liquidityScore: .8, flowScore: .8, buyerGrowthScore: .8, manipulationPenalty: 0, reasons: [] }, supplyControl: { score: 0, deployerRisk: 0, concentrationRisk: 0, bundledSupplyRisk: 0, liquidityControlRisk: 0, reasons: [] }, holderCohort: { score: .8, profitableTrackedWallets: 0, accumulatingWallets: 0, distributingWallets: 0, reasons: [] }, attention: { score: .8, crossSourceConfirmation: .8, engagementQuality: .8, sourceCredibility: .8, manipulationPenalty: 0, reasons: [] }, strategyFit: { score: .8, matchedSignals: [], conflicts: [] }, riskAssessment: { marketIntegrity: .1, liquidityRisk: .1, supplyControlRisk: 0, holderConcentrationRisk: .1, walletCohortRisk: 0, socialManipulationRisk: 0, narrativeFragilityRisk: .1, developerRisk: 0, contractRisk: 0, networkRisk: .1, attentionQuality: .8, exitLiquidityRisk: .1, overallRisk: .1, band: 'candidate' as const }, thesis: 'test', invalidation: { conditions: ['liquidity loss'], severity: 'high' as const }, positionPlan: { maxPositionFraction: .01, entryConditions: [], profitTakingConditions: [], exitConditions: [] }, confidence: .5, evidenceIds: ['m3'], assessmentVersion: 'test'
    }
    const withdrawal: LPWithdrawalAttribution = { signature: 'SIG', observedAt: '2026-01-01T00:00:00Z', poolAddress: 'POOL', lpMint: 'LP', lpTokenAccount: 'LP_ACCOUNT', lpAmountRaw: 1000n, ownerBefore: 'OTHER', lpStateEventId: 'lp-event', developerAssociation: 'NOT_MATCHED', evidenceIds: ['other'], confidence: 1 }
    const lowConfidence: LPWithdrawalAttribution = { ...withdrawal, developerAssociation: 'MATCHED', confidence: .69, evidenceIds: ['low'] }
    const enriched = applyActorRiskIntelligence(base, { verifiedLPWithdrawals: [withdrawal, lowConfidence] })
