...
export interface MatchDecision { workerId: ID; jobId: ID; score: number; reasons: string[]; unmetRequirements: string[]; }
export interface Referral { id: ID; workerId: ID; jobId: ID; agencyId: ID; consentId: ID; match: MatchDecision; status: ReferralStatus; }
export interface Placement { id: ID; referralId: ID; workerId: ID; agencyId: ID; employerId: ID; jobId: ID; agreedPayRate: number; currency: string; startsAt: string; status: PlacementStatus; }
export interface Assignment { id: ID; placementId: ID; schedule: string; supervisorId?: ID; status: AssignmentStatus; createdAt?: string; }
export interface Timesheet { id: ID; assignmentId: ID; workerId: ID; periodStart: string; periodEnd: string; hours: number; status: TimesheetStatus; submittedAt?: string; }
