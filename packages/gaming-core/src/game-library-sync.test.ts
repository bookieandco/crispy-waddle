...
describe('syncGameSources',()=>{
 ...
 it('updates metadata without losing user favorite state',async()=>{const repo=new InMemoryGameLibrary();await syncGameSources(repo,[source]);const existing=await repo.get('moonlight:homebase:steam');expect(existing).toBeDefined();await repo.save({...existing!,favorite:true});const result=await syncGameSources(repo,[{...source,contentUri:'steam://updated'}]);expect(result.updated).toBe(1);expect((await repo.get('moonlight:homebase:steam'))?.favorite).toBe(true);});
});
