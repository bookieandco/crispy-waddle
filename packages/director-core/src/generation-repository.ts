...
export interface GenerationRepository {
  ...
  listSubmissionRecoveryCandidates(limit: number): Promise<GenerationSubmissionOutbox[]>;
}
...
export class InMemoryGenerationRepository implements GenerationRepository {
  ...
  async acknowledgeSubmissionAndSaveState(submissionId: string, workerId: string, submissionLeaseToken: string, executionLeaseToken: string, providerJobId: string, task: GenerationTask, execution: GenerationExecution): Promise<AtomicSubmissionAcknowledgement | undefined> {
    const submission = this.submissions.get(submissionId); const durableTask = this.tasks.get(task.id); const durableExecution = this.executions.get(execution.id); const now = Date.now();
    ...
    this.submissions.set(submission.id, clone(acknowledged)); this.executions.set(execution.id, clone(execution)); this.tasks.set(task.id, clone(task));
  ...
  async listSubmissionRecoveryCandidates(limit: number): Promise<GenerationSubmissionOutbox[]> { const now = Date.now(); return [...this.submissions.values()].filter((submission) => submission.status === 'recovery_required' || submission.status === 'pending' || (submission.status === 'submitting' && !!submission.leaseExpiresAt && Date.parse(submission.leaseExpiresAt) <= now)).sort((a, b) => Date.parse(a.updatedAt) - Date.parse(b.updatedAt)).slice(0, Math.max(0, limit)).map(clone); }
