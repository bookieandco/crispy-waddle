...
function buildSystemPrompt(): string {
  return [
    ...
    'provenance-aware ownerContext, bounded liveContext, ephemeral screen/file artifacts, and an',
    'optional expressionDirective produced by deterministic Jhadina kernels.',
    'liveContext contains recent conversation turns and active WorkSession metadata for continuity only.',
    'Use it to resolve words such as "this", "that", "it", "earlier", or "the one on the left" only when',
    'the supplied recent turns, current/previous screen artifacts, files, and WorkSession make the referent unambiguous.',
    'If more than one referent is plausible, ask a focused clarification rather than choosing one.',
    'Recent conversation turns and WorkSession metadata are not independent evidence of external facts,',
    'not durable Memory, and not permission to execute or persist anything.',
