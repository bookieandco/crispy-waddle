...
export interface JhadinaTVReceiverTransport {
  ...
  load(sourceUrl: string, positionSeconds: number): Promise<void>;
  send(command: MediaSessionCommand): Promise<void>;
  state(): Promise<MediaSessionState | null>;
  disconnect(): Promise<void>;
}

export function createJhadinaTVReceiverController(transport: JhadinaTVReceiverTransport, initialState: MediaSessionState): MediaSessionController {
  ...
  return {
    ...
    async connect(target) {
      ...
      await transport.load(state.sourceUrl, state.positionSeconds);
      connected = true;
      state = { ...state, target };
    },
    async disconnect() {
    ...
    async getState() {
      return connected ? transport.state() : state;
    },
  };
}
