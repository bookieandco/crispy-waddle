...
const fakeWindow = {
  addEventListener(type: string, listener: () => void) { const bucket = listeners.get(type) ?? new Set<() => void>(); bucket.add(listener); listeners.set(type, bucket); },
  removeEventListener(type: string, listener: () => void) { listeners.get(type)?.delete(listener); },
};
const fakeDocument = {
  visibilityState: 'visible',
  addEventListener(type: string, listener: () => void) { const bucket = listeners.get(`document:${type}`) ?? new Set<() => void>(); bucket.add(listener); listeners.set(`document:${type}`, bucket); },
...
const item = (id: string): MediaQueueItem => ({ id, titleId: id, title: id, kind: 'movie', playback: { providerId: 'direct', source: { id: `source:${id}`, titleId: id, kind: 'hls', url: `https://example.com/${id}.m3u8` }, capabilities: ['playback', 'seek'] } });
...
function fakeSession(initial: MediaSessionState) {
  ...
  const session = {
    getState: () => state,
    getAuthorityGeneration: () => authorityGeneration,
    ...
    bumpAuthority: () => { authorityGeneration += 1; },
  } as unknown as UnifiedMediaSession & { emit(next: MediaSessionState): void; bumpAuthority(): void };
  return session;
}
...
describe('runtime-owned media playback progress', () => {
  ...
  it('does not persist an old item snapshot under the next queue item during navigation', async () => {
    ...
    const writes: MediaPlaybackProgress[] = [];
    const client = { upsert: vi.fn(async (progress: MediaPlaybackProgress) => { writes.push(progress); if (writes.length === 1) await firstWrite; return progress; }) };
  ...
  it('preserves a completion snapshot when authority changes before the completion write', async () => {
    const session = fakeSession({ titleId: 'one', kind: 'movie', sourceUrl: 'https://example.com/one.m3u8', positionSeconds: 120, durationSeconds: 120, playing: false });
    const writes: MediaPlaybackProgress[] = [];
    let releaseWrite: (() => void) | undefined;
    const blocked = new Promise<void>((resolve) => { releaseWrite = resolve; });
    const client = { upsert: vi.fn(async (progress: MediaPlaybackProgress) => { writes.push(progress); if (writes.length === 1) await blocked; return progress; }) };
    const persistence = attachRuntimeProgressPersistence({ session, getCurrentItem: () => item('one'), userId: 'user-a', client });

    const ordinary = persistence.flush(false);
    await Promise.resolve();
    session.bumpAuthority();
    const completion = persistence.flush(true);
    releaseWrite!();
    await Promise.all([ordinary, completion]);

    expect(writes).toHaveLength(2);
    expect(writes[1]).toMatchObject({ itemId: 'one', positionMs: 120000, completed: true });
    persistence.dispose();
  });

  it('captures completion for item A before item B becomes current', async () => {
    ...
    const completion = persistence.flush(true);
    current = item('b');
    session.emit({ titleId: 'b', kind: 'movie', sourceUrl: 'https://example.com/b.m3u8', positionSeconds: 0, durationSeconds: 90, playing: false });
    await completion;
