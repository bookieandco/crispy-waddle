...
export interface SubsystemIntelligenceRequest {readonly actorId:string;readonly assetId:string;readonly evidence:readonly EvidenceRef[];readonly uncertainty:readonly string[];readonly intent?:string;}
...
export class GovernedSubsystemDispatcher{
 ...
 async dispatch(packet:AssetIntelligencePacket,intent?:string):Promise<SubsystemDispatchResult>{
  ...
  for(const route of packet.routing.routes){
   ...
   const response=await adapter.ingest({actorId:packet.asset.actorId,assetId:packet.asset.id,evidence:packet.evidence,uncertainty:packet.uncertainty,intent});
