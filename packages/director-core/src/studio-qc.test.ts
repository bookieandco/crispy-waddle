...
describe('Director Studio QC',()=>{
 ...
 it('passes complete evidence and emits canonical QC receipts',async()=>{
  ...
  expect(result.outputAssetIds).toEqual(['shot'])
  expect(result.evidenceIds).toContain('qc-report:report-1')
  expect(result.evidenceIds).toContain('sync-qc')
  expect(result.evidenceIds).toContain('qc-check:tracking:0.95')
  expect(result.evidenceIds).toContain('qc-check:voice-sync:0.9')
 })
