...
export type SharkStreetSignal = {
  name: 'liquidity_flight' | 'insider_accumulation' | 'insider_distribution' | 'holder_concentration' | 'volume_anomaly' | 'social_hype_without_depth' | 'developer_behavior' | 'migration_behavior' | 'wash_activity' | 'failed_sellability'
...
export type SharkOpportunityDecision = {
  ...
  policy: { policyPassed: boolean; authorizationRequired: boolean; authorized: boolean; executionPermitted: false }
  decidedAt: string
  modelVersion?: string
}
...
const riskFlagMap: Record<string, SharkRisk> = {
  liquidity: 'liquidity', holder_concentration: 'holder_concentration', contract_control: 'contract_control', sellability: 'sellability', market_structure: 'market_structure', social_manipulation: 'social_manipulation', wallet_behavior: 'wallet_behavior', source_quality: 'source_quality', execution: 'execution',
}
...
export function assessOpportunity({ opportunity, kind = 'other', streetSignals = [], modelVersion }: SharkOpportunityAssessmentInput): SharkOpportunityDecision {
  const risks = new Set<SharkRisk>()
  for (const flag of opportunity.riskFlags) risks.add(riskFlagMap[flag.toLowerCase().replace(/\s+/g, '_')] ?? 'unknown')
  for (const claim of opportunity.claims) {
    const risk = riskFromClaim(claim)
    if (risk) risks.add(risk)
  }
  const evidence = opportunity.evidence.map(adaptOpportunityEvidence)
  const sourceQuality = clampScore(opportunity.sourceConfidence)
  const confidence = clampScore(opportunity.opportunityScore !== undefined ? opportunity.opportunityScore / 100 : sourceQuality)
  const severeRisk = risks.has('liquidity') || risks.has('sellability') || risks.has('contract_control')
  const decision: SharkDecision = severeRisk ? 'avoid' : evidence.length === 0 ? 'observe' : opportunity.verificationStatus === 'verified' ? 'candidate' : 'needs_human_review'
  return {
    id: `shark:${opportunity.id}`, opportunityId: opportunity.id, kind, decision, confidence, expectedValue: opportunity.expectedValue,
    downsideRisk: severeRisk ? 1 : risks.size / 10, risks: [...risks], evidence, streetSignals, sourceQuality, novelty: 0, repeatability: 0,
    policy: { policyPassed: false, authorizationRequired: true, authorized: false, executionPermitted: false }, decidedAt: new Date().toISOString(), modelVersion,
  }
}
...
export function validateSharkDecision(decision: SharkOpportunityDecision): string[] {
  ...
  for (const [name, value] of [['confidence', decision.confidence], ['sourceQuality', decision.sourceQuality], ['novelty', decision.novelty], ['repeatability', decision.repeatability]] as const) {
...
export type { SharkEvidenceAdapter } from './evidence-adapter.js'
