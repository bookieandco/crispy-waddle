...
import { createArtifactIdentity, createProvenanceSnapshot, detectProvenanceDrift, traceProvenance, validateProductionArtifact } from './provenance-assurance.js';

describe('provenance assurance', () => {
  it('creates a deterministic artifact identity and validates it', () => {
    const identity = createArtifactIdentity({ artifactId: 'artifact:1', artifactType: 'runtime', artifactHash: 'sha256:artifact', sourceHash: 'sha256:source', contractHash: 'sha256:contract' });
