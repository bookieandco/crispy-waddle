...
describe('createComfyUIHttpClient', () => {
  it('queues a workflow through ComfyUI /prompt', async () => {
    const fetchImpl = vi.fn<typeof fetch>(async () => new Response(JSON.stringify({ prompt_id: 'job-123' }), { status: 200 }));
  ...
  it('propagates a stable client idempotency key to ComfyUI', async () => {
    const fetchImpl = vi.fn<typeof fetch>(async () => new Response(JSON.stringify({ prompt_id: 'job-456' }), { status: 200 }));
    const client = createComfyUIHttpClient({ baseUrl: 'http://localhost:8188', fetchImpl });
  ...
  it('returns undefined when no prompt matches the stable client idempotency key', async () => {
    const fetchImpl = vi.fn<typeof fetch>(async () => new Response(JSON.stringify({ old: { prompt: [1, 'other-key', {}] } }), { status: 200 }));
    const client = createComfyUIHttpClient({ baseUrl: 'http://localhost:8188', fetchImpl });
  ...
  it('unwraps a prompt history entry', async () => {
    const fetchImpl = vi.fn<typeof fetch>(async () => new Response(JSON.stringify({ 'job-123': { outputs: { '1': {} } } }), { status: 200 }));
    const client = createComfyUIHttpClient({ baseUrl: 'http://localhost:8188', fetchImpl });
