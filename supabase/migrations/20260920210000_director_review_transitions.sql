...
create table if not exists public.director_review_transitions(
 decision_id text primary key references public.director_media_review_decisions(id) on delete restrict, project_id text not null,
 review_stage_id text not null, review_stage_version bigint not null, generation_stage_id text not null, generation_stage_version bigint not null,
 successor_generation_stage_id text, reason text not null, created_at timestamptz not null default now()
);
alter table public.director_review_transitions enable row level security;
revoke all on public.director_review_transitions from anon,authenticated;
grant select,insert on public.director_review_transitions to service_role;
create or replace function public.apply_director_review_transition(p_decision_id text,p_project_id text,p_review_stage_id text,p_review_version bigint,p_generation_stage_id text,p_generation_version bigint,p_review_status text,p_generation_status text,p_successor_version bigint,p_reason text)
...
begin
 ...
 if p_successor_version is not null then successor_id:=g.id||':v'||p_successor_version;
  ...
  values(successor_id,g.project_id,'generation',g.depends_on,'ready',g.input_artifact_ids,'{}',p_successor_version,jsonb_build_object('reason',p_reason,'decisionId',p_decision_id,'previousStageId',g.id)) on conflict(id) do nothing;
 end if;
