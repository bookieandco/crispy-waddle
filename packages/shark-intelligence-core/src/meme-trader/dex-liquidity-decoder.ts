...
export abstract class BaseDexLiquidityDecoder implements DexLiquidityDecoder {
  ...
  protected static pumpSwapReserveState(raw: unknown): { baseReserve: number; quoteReserve: number; liquidityUsd: number; kind: NormalizedReserveState['kind'] } | undefined {
    ...
    // PumpSwap effective quote reserves are raw vault + virtual quote reserves.
    // When virtual reserves are non-zero, a liquidity value derived from raw vault
    // balances is not admissible as canonical liquidity truth.
    return undefined
  }
}
