...
import type { SpatialObservation } from './reality.js'
...
function assertCoordinate(lat: number, lon: number): void {
  if (!Number.isFinite(lat) || lat < -90 || lat > 90 || !Number.isFinite(lon) || lon < -180 || lon > 180) {
    throw new Error('GEV_CAMERA_COORDINATE_INVALID')
  }
}
  ...
  toEvidence(snapshot, observationId, contentHash) {
    ...
    return {
      ...
      source: {
        ...
        record_id: snapshot.sourceRecordId ?? camera.id,
        attribution: camera.attribution ?? 'God’s Eye View source adapter',
      },
      ...
      payload: {
        entity: { id: camera.id, type: 'camera' },
        position: { lat: camera.lat, lon: camera.lon },
        attributes: { frameRef: snapshot.frameRef, name: camera.name ?? null },
      },
