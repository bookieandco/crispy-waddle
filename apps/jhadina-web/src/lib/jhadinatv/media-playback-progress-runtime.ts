...
export interface RuntimeProgressClient {
  upsert(progress: MediaPlaybackProgress): Promise<MediaPlaybackProgress>;
}

export interface RuntimeProgressPersistence {
  flush(completed?: boolean): Promise<void>;
  dispose(): void;
}

export interface RuntimeProgressPersistenceDeps {
...
export function createMediaPlaybackProgressApiClient(fetchImpl: typeof fetch = fetch): RuntimeProgressClient {
  return {
    async upsert(progress) {
      ...
      if (!response.ok) throw new Error(`JHADINA_MEDIA_PLAYBACK_PROGRESS_FAILED:${response.status}`);
      const payload = (await response.json()) as { progress?: MediaPlaybackProgress };
      if (!payload.progress) throw new Error('JHADINA_MEDIA_PLAYBACK_PROGRESS_FAILED:no_result');
...
export function attachRuntimeProgressPersistence({ session, getCurrentItem, userId, client, throttleMs = 5000, onError }: RuntimeProgressPersistenceDeps): RuntimeProgressPersistence {
  ...
  const requestWrite = (completed = false): Promise<void> => {
    ...
    const inferredCompleted = completed || (
      !state.playing &&
    ...
    pending = {
      item,
      state,
      completed: inferredCompleted || pending?.completed === true,
    };
  ...
  const unsubscribe = session.subscribe((state) => {
    if (disposed) return;
    const becamePaused = lastPlaying && !state.playing;
    lastPlaying = state.playing;
    if (becamePaused) void requestWrite(false);
    else if (state.playing) schedule();
  });
  ...
  const flush = async (completed = false): Promise<void> => {
    ...
    while (!disposed && (inFlight || pending)) {
