...
export interface ResourceEnforcementReceipt { executionId: string; limitsDigestSha256: string; dimensions: Readonly<Record<ResourceDimension, ResourceEnforcementStatus>>; }
...
export interface RuntimeResourceLease { executionId: string; limits: ResourceLimits; enforcement: ResourceEnforcementReceipt; release(): Promise<void>; }
...
export function assertExecutionRequest(request: ExecutionRequest): void { if (!request.actorId.trim()) throw new Error('Runtime actor binding is required'); if (!request.executionId.trim()) throw new Error('Runtime executionId is required'); if (request.artifact.trust !== 'trusted') throw new Error('Untrusted program artifacts cannot be executed'); assertManifestMatchesArtifact(request.artifact, request.manifest); const expected = deriveExecutionId({ actorId: request.actorId, artifactDigestSha256: request.artifact.digestSha256, manifestId: request.manifest.manifestId, requestedAt: request.requestedAt }); if (request.executionId !== expected) throw new Error('Runtime executionId must match deterministic request identity'); const declared = new Set(request.manifest.requestedCapabilities); const granted = new Set(request.capabilityGrants.map((grant) => grant.capability)); if (declared.size !== request.manifest.requestedCapabilities.length || granted.size !== request.capabilityGrants.length) throw new Error('Runtime capabilities and grants must be unique'); if (declared.size !== granted.size || [...declared].some((capability) => !granted.has(capability))) throw new Error('Runtime capability grants must exactly bind declared capabilities'); for (const grant of request.capabilityGrants) if (!grant.grantId.trim()) throw new Error('Runtime capability grant id is required'); }
export class InMemoryExtensionPatchRegistry implements ExtensionPatchRegistry { private readonly patches = new Map<string, ExtensionPatchRegistration>(); register(patch: ExtensionPatchRegistration): void { if (!patch.patchId || !patch.extensionId || !patch.targetFingerprint || !patch.artifactDigestSha256) throw new Error('Incomplete extension patch registration'); if (this.patches.has(patch.patchId)) throw new Error(`Duplicate extension patch: ${patch.patchId}`); this.patches.set(patch.patchId, Object.freeze({ ...patch, before: Object.freeze([...(patch.before ?? [])]), after: Object.freeze([...(patch.after ?? [])]), requestedCapabilities: Object.freeze([...patch.requestedCapabilities]) })); } list(): readonly ExtensionPatchRegistration[] { return [...this.patches.values()]; } }
...
export { assertSandboxIsolationEvidence, SandboxProviderResourceEnforcer, SandboxProviderRuntimeAdapter } from './sandbox-provider.js';
export type { SandboxProviderKind, SandboxControlState, SandboxIsolationEvidence, SandboxLease, SandboxProviderPort } from './sandbox-provider.js';
