    ...
    async function load() {
      try {
        ...
        if (musicRes.ok) {
          ...
          const musicItems: FeedItem[] = json.data ?? [];
          next.push(...musicItems);
        }
...
export function PersonalCommandFeed() {
  ...
  const filters: Array<{ id: typeof filter; label: string }> = [
    { id: 'all', label: 'All' }, { id: 'work', label: 'Work' }, { id: 'opportunity', label: 'Opportunities' }, { id: 'media', label: 'Music' },
