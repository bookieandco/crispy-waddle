...
export default function PetIdentityUploader({ productId, artStyle, onCompleted }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const enqueueResponse = await fetch("/api/pet-creative-job", {
        method: "POST",
        body: form,
      });
      const enqueue = await enqueueResponse.json();

      if (!enqueueResponse.ok || !enqueue.success) {
        throw new Error(enqueue.error ?? "Could not save the pet identity.");
      }

      setStatus("Creating three designs…");

      const runResponse = await fetch(`/api/pet-creative-job/${enqueue.job.id}/run`, {
        method: "POST",
      });
      const run = await runResponse.json();

      if (!runResponse.ok || !run.success) {
        throw new Error(run.error ?? "The creative job failed.");
      }

      setStatus("Your designs are ready.");
      onCompleted(run.outputs ?? []);
    } catch (err) {
    ...
    <section className="mb-6 rounded-xl border border-greige/50 bg-white/40 p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-ink/50">Add up to 5 photos for a better likeness.</p>
        </div>
      ...
      <input
        ...
        maxLength={80}
        className="mb-3 w-full rounded-md border border-greige/50 bg-cream px-3 py-2 text-sm outline-none focus:border-honey-oak"
      />

      <textarea
        value={prompt}
        onChange={(event) => setPrompt(event.target.value)}
        placeholder="Describe what you want… or leave it blank."
        maxLength={2000}
        rows={3}
        className="mb-3 w-full resize-none rounded-md border border-greige/50 bg-cream px-3 py-2 text-sm outline-none focus:border-honey-oak"
      />
      ...
      <div className="mb-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            ["keep", "Keep photo"],
            ["generate", "Describe one"],
          ].map(([value, label]) => (
            <button
              key={value}
              type="button"
              onClick={() => setBackgroundMode(value as typeof backgroundMode)}
              className={`rounded-md border px-2 py-2 text-xs transition ${
                backgroundMode === value
                  ? "border-honey-oak bg-honey-oak text-cream"
                  : "border-greige/50 text-ink/60 hover:border-honey-oak"
              }`}
            >
              {label}
            </button>
      ...
      <button
        ...
        className="w-full rounded-md bg-bronze py-3 text-sm font-semibold text-cream transition hover:bg-ink disabled:cursor-not-allowed disabled:opacity-40"
      >
        {busy ? "Creating…" : "Create 3 designs"}
      </button>
    </section>
  );
}
