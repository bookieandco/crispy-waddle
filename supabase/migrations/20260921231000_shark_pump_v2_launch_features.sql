-- SHARK-CONVERGE: persist Pump 2 launch semantics without changing outcome authority.
alter table public.jhadina_token_launches
  add column if not exists pump_features jsonb;

alter table public.jhadina_token_launches
  drop constraint if exists jhadina_token_launches_pump_features_object_check;
alter table public.jhadina_token_launches
  add constraint jhadina_token_launches_pump_features_object_check
  check (pump_features is null or jsonb_typeof(pump_features) = 'object');
...
begin
  ...
  insert into public.jhadina_token_launches (
    ...
    cluster_id, launched_at, launchpad, initial_liquidity_usd, pump_features, outcome,
    ...
    case when p_launch ? 'pump_features' and p_launch->'pump_features' <> 'null'::jsonb then p_launch->'pump_features' else null end,
  ...
  on conflict (chain_id, token_address) do update set
    ...
    pump_features = coalesce(excluded.pump_features, jhadina_token_launches.pump_features),
    evidence_ids = (
