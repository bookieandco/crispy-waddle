...
export class HomeAssistantRemoteTransport implements RemoteTransport {
  ...
  constructor(private readonly config: HomeAssistantRemoteTransportConfig) {}

  supports(command: ResolvedRemoteCommand): boolean {
    return command.capability === POWER_CAPABILITY;
  }

  async execute(command: ResolvedRemoteCommand): Promise<void> {
    if (!this.supports(command)) throw new Error('unsupported-capability');

    const fetchImpl = this.config.fetchImpl ?? fetch;
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), this.config.timeoutMs ?? 5000);
    try {
      const response = await fetchImpl(`${this.config.baseUrl.replace(/\/$/, '')}/api/services/homeassistant/toggle`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${this.config.authToken}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ entity_id: this.config.entityId }),
