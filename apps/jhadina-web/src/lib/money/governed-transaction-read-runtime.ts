...
export type GovernedTransactionRuntimeOverrides={identityVerifier?:JhadinaIdentityVerifier;supabase?:AuditRpcClient;providers?:MoneyPlaidProductionRegistry;ownedAccountIds?:ReadonlySet<string>}
...
export async function runSessionGovernedMoneyTransactionRead(accountId:string,requestId:string,overrides:GovernedTransactionRuntimeOverrides={}):Promise<{transactions:readonly MoneyTransaction[];verifiedUserId:string}>{
  ...
  const owned=overrides.ownedAccountIds??new Set<string>();
