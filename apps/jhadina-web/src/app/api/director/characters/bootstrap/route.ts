...
function cleanText(value: unknown, maxLength = 8000): string | undefined {
...
function cleanNotes(value: unknown): string[] {
  ...
  const notes = [...new Set(value.filter((item): item is string => typeof item === 'string').map((item) => item.replace(/\s+/g, ' ').trim()).filter(Boolean))];
  if (notes.some((note) => note.length > 500)) throw new Error('DIRECTOR_CAST_PERFORMANCE_NOTE_TOO_LONG');
  return notes;
}
...
export async function POST(request: Request) {
  ...
  try {
    characterDescription = cleanText(body.characterDescription);
    appearanceDescription = cleanText(body.appearanceDescription);
    performanceNotes = cleanNotes(body.performanceNotes);
  } catch (error) {
    return NextResponse.json({
      ok: false,
      error: error instanceof Error ? error.message : 'DIRECTOR_CAST_DESCRIPTION_INVALID',
    }, { status: 400 });
  }
  ...
  const cast: CharacterCastRecord = {
    ...
    ...(performanceNotes.length ? { performanceNotes } : {}),
    descriptionRevision: 1,
    descriptionUpdatedAt: now,
    descriptionUpdatedBy: user.id,
    canonicalAppearanceVariantId: baseVariantId,
    appearanceVariants: [{
      id: baseVariantId,
      characterId,
      kind: 'base',
      label: 'Canonical uploaded reference sheet',
      referenceAssetIds,
      referenceSha256s: uploads.map((upload) => upload.sha256),
      ...(appearanceDescription ? { appearanceNotes: [appearanceDescription] } : {}),
      approvedAt: now,
      approvedBy: user.id,
    }],
    lockedTraits,
  ...
  const { data, error } = await privileged.rpc('create_director_character_bootstrap_job', {
    ...
    p_continuity_ref: plan.continuityRef,
    p_base_variant_id: baseVariantId,
    p_reference_asset_ids: referenceAssetIds,
    p_reference_sha256s: uploads.map((upload) => upload.sha256),
    p_locked_traits: lockedTraits,
    p_character_description: characterDescription ?? '',
    p_appearance_description: appearanceDescription ?? '',
    p_performance_notes: performanceNotes,
  ...
  return NextResponse.json({
    ...
    canonicalReferenceSheetId: baseVariantId,
    descriptionRevision: 1,
    bootstrapJob: data,
    status: 'reference_locked',
    next: 'reference-aware video generation may start immediately; written descriptions and reference-sheet assets now travel together',
  }, { status: 201 });
}
