...
BEGIN
  ...
  SELECT * INTO retry_permit FROM public.money_execution_permits WHERE permit_id=new.permit_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'MONEY_RECOVERY_PERMIT_NOT_FOUND'; END IF;
  SELECT * INTO parent_permit FROM public.money_execution_permits WHERE permit_id=p.permit_id;
  ...
  SELECT observed_state,proposal_hash,evidence_hash,checked_at
  ...
  IF latest_reconciliation.observed_state<>'NOT_FOUND' THEN RAISE EXCEPTION 'MONEY_RECOVERY_RETRY_NOT_SAFE'; END IF;
  IF latest_reconciliation.proposal_hash<>p.action_fingerprint THEN RAISE EXCEPTION 'MONEY_RECOVERY_RETRY_FINGERPRINT_MISMATCH'; END IF;
...
BEFORE INSERT OR UPDATE OF recovery_of_execution_id,request_id,permit_id,action_fingerprint,provider,operation
