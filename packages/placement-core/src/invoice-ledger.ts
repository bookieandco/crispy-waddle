...
export interface RevenueLedgerEntry {
  ...
  organizationId: ID;
  invoiceId: ID;
  type: "INVOICE_ISSUED" | "PAYMENT_RECEIVED" | "PLATFORM_FEE" | "REFUND";
  amount: number;
  currency: string;
  occurredAt: string;
}
...
export interface InvoiceRepository {
  ...
  issueInvoice(id: ID): Promise<Invoice>;
  savePayment(payment: Payment): Promise<void>;
  updateInvoiceStatus(id: ID, status: InvoiceStatus): Promise<Invoice>;
  saveLedgerEntry(entry: RevenueLedgerEntry): Promise<void>;
}
...
export class InvoiceLedgerService {
  constructor(
    ...
    private readonly ids: InvoiceIds,
    private readonly clock: InvoiceClock,
  ) {}

  async draftFromBilling(input: {
    organizationId: ID;
    employerId: ID;
    agencyId: ID;
    timesheetId: ID;
    ...
    const invoice: Invoice = {
      ...
      subtotal: input.billing.employerBill,
  ...
  async recordPayment(input: Omit<Payment, "id" | "status">): Promise<Payment> {
    if (input.amount <= 0) throw new Error("Payment amount must be positive");
    const payment: Payment = { ...input, id: this.ids.next("payment"), status: "SETTLED", receivedAt: this.clock.now() };
