...
function accountKeys(raw: Record<string, unknown>): string[] {
  const tx = object(raw.transaction) ?? raw
  const message = object(tx.message)
  const staticKeys = Array.isArray(message?.accountKeys) ? message.accountKeys : Array.isArray(tx.accountKeys) ? tx.accountKeys : []
  const keys = staticKeys.map(key => typeof key === 'string' ? key : (object(key)?.pubkey as string | undefined)).filter((key): key is string => typeof key === 'string')
  const meta = object(raw.meta)
  const loaded = object(meta?.loadedAddresses)
  const writable = Array.isArray(loaded?.writable) ? loaded.writable.filter((v): v is string => typeof v === 'string') : []
  const readonly = Array.isArray(loaded?.readonly) ? loaded.readonly.filter((v): v is string => typeof v === 'string') : []
  return [...keys, ...writable, ...readonly]
}

function accountKey(raw: Record<string, unknown>, index: number): string | undefined { return accountKeys(raw)[index] }
...
function balances(raw: Record<string, unknown>, key: 'preTokenBalances' | 'postTokenBalances', lpMint: string): Balance[] {
  ...
  return rows.flatMap(value => {
    ...
    if (!row || row.mint !== lpMint || typeof row.accountIndex !== 'number') return []
    const tokenAmount = object(row.uiTokenAmount)
    const amountRaw = parseAmount(tokenAmount?.amount ?? row.amount)
    return amountRaw === undefined ? [] : [{ accountIndex: row.accountIndex, mint: lpMint, owner: typeof row.owner === 'string' ? row.owner : undefined, amountRaw }]
  })
}

function parsedTokenOperations(raw: Record<string, unknown>, lpMint: string): TokenOperation[] {
  const meta = object(raw.meta)
  const groups = Array.isArray(meta?.innerInstructions) ? meta.innerInstructions as unknown[] : []
  const result: TokenOperation[] = []
  for (const group of groups) {
    ...
    instructions.forEach((value, index) => {
      ...
      if (!info || (program !== 'spl-token' && program !== 'spl-token-2022') || !type) return
      const mint = typeof info.mint === 'string' ? info.mint : undefined
      if (mint !== lpMint) return
      const amountRaw = parseAmount(info.amount ?? object(info.tokenAmount)?.amount)
      const evidence = `inner:${String(g?.index ?? 'unknown')}:${index}`
      if (type === 'mintTo' || type === 'mintToChecked') result.push({ kind: 'MINT', account: typeof info.account === 'string' ? info.account : undefined, amountRaw, evidence })
      else if (type === 'burn' || type === 'burnChecked') result.push({ kind: 'BURN', account: typeof info.account === 'string' ? info.account : undefined, amountRaw, evidence })
      else if (type === 'transfer' || type === 'transferChecked') result.push({ kind: 'TRANSFER', source: typeof info.source === 'string' ? info.source : undefined, destination: typeof info.destination === 'string' ? info.destination : undefined, amountRaw, evidence })
    })
  }
  return result
}
...
export class SolanaLPStateMachine {
  decode(transaction: HistoricalPoolTransaction, pool: PoolHistory['pool'], lpMint: string): LPPositionEvent[] {
    ...
    for (const [index, state] of byIndex) {
      ...
      if (after > before) {
        const mint = matching.find(op => op.kind === 'MINT')
        const transfer = matching.find(op => op.kind === 'TRANSFER' && op.destination === address)
        if (mint && mint.amountRaw === delta) { kind = 'MINT'; corroborated = true }
        else if (transfer && transfer.amountRaw === delta) { kind = 'TRANSFER'; corroborated = true }
      } else {
        const burn = matching.find(op => op.kind === 'BURN')
        const transfer = matching.find(op => op.kind === 'TRANSFER' && op.source === address)
        if (burn && burn.amountRaw === delta) { kind = 'BURN'; corroborated = true }
        else if (transfer && transfer.amountRaw === delta) { kind = 'TRANSFER'; corroborated = true }
      }
      ...
      events.push({
        ...
        lpMint,
        kind,
        from: before > after ? state.pre?.owner : undefined,
        to: after > before ? state.post?.owner : undefined,
        amountRaw: delta,
        source: 'solana-token-balance-reconciliation',
        evidenceIds: [...new Set([transaction.evidenceId, evidenceId(transaction, `lp:${index}`), ...matchingEvidence])],
...
export function buildLPStateMachineEvents(input: { history: PoolHistory; lpMint: string }): LPPositionEvent[] {
  return input.history.transactions.flatMap(tx => new SolanaLPStateMachine().decode(tx, input.history.pool, input.lpMint)).sort((a, b) => Date.parse(a.observedAt) - Date.parse(b.observedAt))
}
