...
let legacyApprovedBlocked = false
try {
  applyOpportunityOutcome({ ...opportunity, status: 'approved' }, outcome, '2026-09-19T13:00:00Z')
} catch {
  legacyApprovedBlocked = true
}
assert(legacyApprovedBlocked, 'Legacy approved state must not authorize realized outcomes')


let blankOutcomeEvidenceBlocked = false
try {
  calculateOpportunityOutcome({
    id: 'outcome:blank-evidence',
    opportunityId: opportunity.id,
    result: 'lost',
    currency: 'USD',
    grossRevenue: 0,
  ...
  blankOutcomeEvidenceBlocked = true
}
assert(blankOutcomeEvidenceBlocked, 'Blank outcome evidence must be rejected')

let blankTransactionRefBlocked = false
try {
  calculateOpportunityOutcome({
    id: 'outcome:blank-transaction',
    opportunityId: opportunity.id,
    result: 'won',
    currency: 'USD',
    grossRevenue: 100,
    ...
    hours: 1,
    sourceOwner: 'money_core',
    evidenceRefs: ['ledger:revenue'],
    transactionRefs: [''],
    observedAt: '2026-09-19T12:00:00Z',
  })
} catch {
  blankTransactionRefBlocked = true
}
assert(blankTransactionRefBlocked, 'Blank transaction lineage refs must be rejected')
