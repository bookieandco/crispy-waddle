...
export class DexScreenerPoolDiscoverySource implements PoolDiscoverySource {
  ...
  async discoverPools(launch: TokenLaunch): Promise<PoolCandidate[]> {
    ...
    const candidates = body.flatMap((raw, index) => {
      ...
      const quoteMint = stringValue(quoteToken?.address)
      ...
      return [{
        ...
        evidenceId: `dexscreener:pool:${launch.chainId}:${launch.tokenAddress}:${poolAddress}:${observedAt}:${index}`,
      }]
    })
    ...
    return candidates.filter(candidate => {
      ...
      seen.add(candidate.poolAddress)
      return true
    })
  }
}
